...
- **Storage tiering** (scope-amended 2026-05-23 PM per HB01 close):
  ...
  - **`/work/ht203/cache/`** (75-day purge, 950 TB cluster / 233 TB free) → bulk feature precomputes cheap to regenerate (Multi-STFT cache ~5–8 TB fp16; v14 per-extractor cache `v14_extractors/<extractor_name>/` ~50–100 GB Lite / ~500 GB–1 TB if SWEC Log-STFT runs there, set via `EXCA_EXTRACTOR_CACHE_FOLDER`; normalization stats). Each such cache MUST ship a documented `MapInfra` regenerate spec — see `docs/neuroprobe/training_recipe.md` for both Multi-STFT and v14-extractor specs.
