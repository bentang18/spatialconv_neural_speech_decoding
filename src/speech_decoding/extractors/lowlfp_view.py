...
class LowLfpView(MultiStftView):
    ...
    64 Hz (Ben 2026-07-21, dropping the earlier 128 Hz bake): matches the HGA |STFT|
    clock (N=64/hop=32 -> 64 Hz) EXACTLY, so LFS frame i == HGA frame i, both streams
    bake at 64 and the EarlyFusionStem conv-pools BOTH 64->32 with the identical
    ``Conv1d(k3,s1)->Conv1d(k3,s2)`` single decimate (symmetric stem; no per-stream
    rate reconcile). LFS is bandlimited to 30 Hz so 64 Hz (Nyquist 32) is
    information-complete -- the earlier 128 Hz bake was 2x-oversampled, carrying no
    new information, so 64 halves the cache and the conv-pool becomes a learned
    temporal filter on the voltage.
    """

    #: LFS frame clock; matches the HGA |STFT| rate so the two streams align
    #: frame-for-frame. ``sample_rate == LFP_TARGET_HZ * hop_length`` (2048 -> hop 32).
    LFP_TARGET_HZ: int = 64
    ...
    def _lfs_frames(
        ...
        window-center grid the HGA STFT (N=64, hop=32) rides, so LFS frame i == HGA
        ...
        # The assert pins that hop to the 64 Hz clock (2048 Hz raw -> hop 32).
        hop = self.hop_length
