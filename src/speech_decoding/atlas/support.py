...
(The B28/B29 soft ``λ_anat·log(support+ε)`` cross-attn bias helper that used
to live here was removed at B36: the encoder now consumes the one-hot DK
``support`` directly as a hard block-diagonal parcel pool — no log smoothing,
no per-clip gate. See ``models/v14_encoder._MultiHeadCrossAttention``.)
"""
