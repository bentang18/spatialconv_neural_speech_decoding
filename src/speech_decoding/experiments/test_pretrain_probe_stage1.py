...
def test_cache_carries_parcel_labels():
    """The compacted parcel grids' DKT ids ride from SessionMeta into the cache (the
    bridge the parcel-space readouts use to map compacted↔atlas lazily)."""
    targets = build_session_targets(_events(2, 1), subject_id=2, trial_id=1)
    cache = cache_from_targets(targets, _stub_forward(targets), META)
    assert torch.equal(cache.parcel_labels, torch.arange(P))
