...
def test_ckpt_ladder_every_is_configurable(tmp_path) -> None:
    """The keep-ALL ladder cadence honors ckpt_ladder_every (default 500). The
    standalone periodic-last and the ladder must always carry UNIQUE Lightning
    state_keys — when their cadences coincide the ladder absorbs save_last and the
    standalone last is dropped, otherwise pl.Trainer rejects the duplicate."""
    import speech_decoding.models  # noqa: F401  # registers TinyMLP discriminator
    from lightning.pytorch.callbacks import ModelCheckpoint
    ...
    def _mcs(xp):
        return [c for c in xp._callbacks() if isinstance(c, ModelCheckpoint)]

    def _ladder(xp):
        return [c for c in _mcs(xp) if c.monitor is None and c.save_top_k == -1]

    def _periodic_last(xp):
        return [c for c in _mcs(xp) if c.monitor is None and c.save_top_k == 0]

    # Default cadence is 500 (run-ops policy).
    assert _ladder(base)[0]._every_n_train_steps == 500

    # Cadences DIFFER → ladder (500) and a standalone periodic-last (1000) coexist;
    # last.ckpt rides val_check_interval, the ladder does not own it.
    differ = base.model_copy(
        update={"val_check_interval": 1000, "ckpt_ladder_every": 500}
    )
    d_ladder = _ladder(differ)[0]
    assert d_ladder._every_n_train_steps == 500 and d_ladder.save_last is False
    d_last = _periodic_last(differ)
    assert d_last and d_last[0]._every_n_train_steps == 1000
    d_keys = [c.state_key for c in _mcs(differ)]
    assert len(d_keys) == len(set(d_keys)), f"colliding state_keys: {d_keys}"

    # Cadences COINCIDE (the regression: ladder == val_check == 1000). The
    # standalone periodic-last is dropped and the ladder owns last.ckpt, so the
    # metric-independent checkpoints stay state_key-unique and Trainer init holds.
    same = base.model_copy(
        update={"val_check_interval": 1000, "ckpt_ladder_every": 1000}
    )
    s_ladder = _ladder(same)[0]
    assert s_ladder._every_n_train_steps == 1000 and s_ladder.save_last is True
    assert not _periodic_last(same), "redundant periodic-last must be dropped"
    s_keys = [c.state_key for c in _mcs(same)]
    assert len(s_keys) == len(set(s_keys)), f"colliding state_keys: {s_keys}"
