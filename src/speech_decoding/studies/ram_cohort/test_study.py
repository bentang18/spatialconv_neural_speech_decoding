"""RamCohortStudy v3-contract tests: manifest-driven timelines + cache-key round-trip.

Glue that needs no mne/EDF data (iter_timelines, _load_timeline_events, _cls_kwargs,
row lookup). _load_raw's EDF read is covered on the staged tree; here only its
pre-read guard is asserted.
"""
...
# Two subjects: one with two runs (trials 0,1), one single-run. RAM ids in 3000-3999.
_ROWS = [
    dict(subject_bids="sub-R1001P", global_subject_id=3001, trial_id=0,
         edf_path="/data/a_ieeg.edf", channels_tsv_path="/data/a_channels.tsv",
         duration_s=1024.0),
    dict(subject_bids="sub-R1001P", global_subject_id=3001, trial_id=1,
         edf_path="/data/b_ieeg.edf", channels_tsv_path="/data/b_channels.tsv",
         duration_s=980.0),
    dict(subject_bids="sub-R1592J", global_subject_id=3142, trial_id=0,
...
def _study(tmp_path, manifest_csv, **kw):
    return RamCohortStudy(path=str(tmp_path), manifest_path=manifest_csv, **kw)


@pytest.fixture
def manifest(tmp_path):
    return _write_manifest(tmp_path / "ram_manifest.csv")


def test_classvars():
    assert RamCohortStudy.SAMPLE_RATE_HZ == 2048.0
    assert "RAM" in RamCohortStudy.aliases
    assert RamCohortStudy.licence.startswith("CC0")
...
def test_load_timeline_events_row_and_cache_key_roundtrip(tmp_path, manifest):
    ...
    assert row["duration"] == 612.0            # from manifest duration_s
    assert row["frequency"] == 2048.0          # post-resample rate, NOT native
    assert parse_key_session(row["filepath"]) == (3142, 0)


def test_session_subset_filters(tmp_path, manifest):
    s = _study(tmp_path, manifest, session_subset=((3001, 1),))
    tls = list(s.iter_timelines())
    assert [(t["subject_id"], t["trial_id"]) for t in tls] == [(3001, 1)]
...
def test_cls_kwargs_drops_enumeration_fields(tmp_path, manifest):
    s = _study(tmp_path, manifest, session_subset=((3001, 0),))
    assert s._cls_kwargs() == {}


def test_manifest_path_resolves_from_env_after_roundtrip(tmp_path, manifest, monkeypatch):
    reconstructed = RamCohortStudy(path=str(tmp_path), manifest_path="")
    monkeypatch.setenv("RAM_MANIFEST", manifest)
...
def test_manifest_path_unset_and_no_env_raises(tmp_path, monkeypatch):
    monkeypatch.delenv("RAM_MANIFEST", raising=False)
    with pytest.raises(ValueError, match="RAM_MANIFEST env is empty"):
        RamCohortStudy(path=str(tmp_path), manifest_path="")._resolved_manifest_path()
...
def test_load_raw_empty_channels_tsv_raises(tmp_path):
    rows = [dict(subject_bids="sub-R1001P", global_subject_id=3001, trial_id=0,
    ...
    with pytest.raises(ValueError, match="no channels.tsv"):
        s._load_raw({"subject_id": 3001, "trial_id": 0})
