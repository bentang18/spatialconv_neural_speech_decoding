"""Tests for LOSS-01: the B28 4-term composer with locked coefficients.

B28 amendment (2026-05-27 PM): DKoleo @ M4 is demoted from the default
composer to an opt-in sister term. Default coefficients are
``(1, 1, 1, 1.0)``; ``W_DKOLEO_M4 = 0.1`` constant is retained for the
``R-dkoleo-{intra_clip_slots,batch_cls}`` sisters but is only applied
when ``l_dkoleo_m4`` is passed.
"""
...
def test_coefficients_pinned() -> None:
    """LOSS-01 lock + B28 amendment 2026-05-27 PM: coefficients are
    ``(1, 1, 1, 1.0)`` always-on; ``0.1`` for the opt-in DKoleo arm."""
    assert W_PRE_FRAME == 1.0, "L_pre_frame coefficient drifted"
    assert W_MID_SLOT == 1.0, "L_mid_slot coefficient drifted"
    assert W_POST_FRAME == 1.0, "L_post_frame coefficient drifted"
    assert W_POST_UTTERANCE == 1.0, "L_post_utterance coefficient drifted"
...
def test_v14_total_loss_default_is_four_terms() -> None:
    """B28 default (2026-05-27 PM): no DKoleo @ M4 in the loss bundle."""
    l_pre = torch.tensor(0.5)
    l_mid = torch.tensor(0.3)
    l_post_f = torch.tensor(0.2)
    l_post_u = torch.tensor(0.4)
    br = v14_total_loss(l_pre, l_mid, l_post_f, l_post_u)
    expected = 1.0 * 0.5 + 1.0 * 0.3 + 1.0 * 0.2 + 1.0 * 0.4
    torch.testing.assert_close(br.total, torch.tensor(expected), atol=1e-6, rtol=1e-6)
    # DKoleo @ M4 is None under the B28 default.
...
def test_v14_total_loss_dkoleo_sister_adds_term_at_locked_weight() -> None:
    ...
    4-term sum and bubbles the per-term tensor + active coefficient."""
    l_pre = torch.tensor(0.5)
    l_mid = torch.tensor(0.3)
    l_post_f = torch.tensor(0.2)
    l_post_u = torch.tensor(0.4)
    ...
    br = v14_total_loss(
        l_pre, l_mid, l_post_f, l_post_u, l_dkoleo_m4=l_dk
    )
    expected = 1.0 * 0.5 + 1.0 * 0.3 + 1.0 * 0.2 + 1.0 * 0.4 + 0.1 * 1.0
    torch.testing.assert_close(br.total, torch.tensor(expected), atol=1e-6, rtol=1e-6)
    assert br.l_dkoleo_m4 is l_dk
    assert br.coefficients == (1.0, 1.0, 1.0, 1.0, 0.1)
...
def test_v14_total_loss_reactive_off_by_default() -> None:
    ...
    br = v14_total_loss(base, base, base, base)
    assert br.l_dkoleo_m3_reactive is None
    assert br.l_gram_reactive is None


def test_v14_total_loss_backward_propagates_through_default_four_terms() -> None:
    """Default composer: gradient flows through the 4 always-on branches
    at the locked unit coefficients."""
...
def test_v14_total_loss_backward_dkoleo_sister_gradient_uses_locked_weight() -> None:
    ...
    br = v14_total_loss(base, base, base, base, l_dkoleo_m4=l_dk)
    br.total.backward()
