...
- **Step-budget axis (operationalizes L0 along the step trajectory).** The recon-L1 plateau is reached early — gate job 47725245 P1 spent ~94% of its post-warmup val-L1 drop by step ~600 (0.257→0.194), then ~0.004 over 600→1500. Per "the one rule" that plateau bounds the *recon* objective, **not** transfer — so sweep the step budget on the gate, never on the loss curve:
  - **Cheap pass (free, within one run):** checkpoint a single cosine-to-max P1 (and P2) run at a step grid (e.g. {600, 900, 1200, 1500}); run the frozen transfer probe at each; pick the best-transfer step. This catches the catastrophic-overtraining U-turn (2503.19206) — the gate can peak *before* the recon plateau's end, i.e. late steps may be net-negative, not merely wasted.
  - **Cosine-horizon caveat:** an intermediate checkpoint of a cosine-to-max run is *under-annealed* at step k<max (LR still well above 0), so it under-states a purpose-built short run. Use the cheap pass to locate a candidate, then **confirm a shortened production budget only with a purpose-built cosine-to-K run** — this is the L1 epochs axis (separate cosine horizon per budget), not free.
  - **Sweep-speed corollary (distinct question):** the recon-side truncation that makes the M0 *ranking* sweep itself cheaper — rank configs at the recon-saturation step rather than the full budget — lives in `training_recipe.md §7 M0 step 4`. That is recon-gap-scored bring-up; this L0 axis is the gate-scored production-budget question. Same observation (early plateau), two different uses.

### L0.5 — Cross-subject latent whitening (label-free, test-time)
- **Cost**: very low (frozen-encoder, eval-side; no retrain)
- **Resolved (2026-06-03)**: lever = **per-subject covariance whitening in the parcel-routed latent space**, NOT raw-channel input EA. Raw-channel EA is ruled out for iEEG — no cross-subject montage correspondence (clinical placement) → per-channel whitening matrices are non-comparable, and it would scramble the electrode→parcel routing v14 depends on. So whitening is **model-side, on the d-dim PMA readout, post-hoc on the frozen encoder (3b)**.
- **Backing**: He & Wu 1808.05464 (EA protocol — per-subject covariance whitening, unsupervised, no target labels; IEEE TBME 2020). Latent-space family: CORAL 1511.05547 (feature-space 2nd-order alignment), SPDDSMBN/TSMNet 2206.01323 (per-domain SPD-manifold BN), Decorrelated-BN 1804.08450 (whitening layer). Mature in CV + scalp-EEG; **zero precedent in iEEG latent space** — a borrowed mechanism with a small honest "first-in-iEEG-latent" edge.
- **Cells**: `R-whiten-latent` (3b contract, P1, leads CSubject) + `R-whiten-global` (non-absorbability control) — `ablations.md` §10. Hypothesis under test = **redundancy**: the anatomy pool + zero per-subject params may already place subjects in a common frame, so a clean null is itself paper-usable. 3a baked-in (SPDDSMBN/Decorrelated-BN inside the forward) deferred P2, only if 3b earns ΔAUROC ≥ 0.02.
- **Open**: EA-revisit 2502.09203 confirmed real (2026-06-03); not yet deep-read.
...
- **Action**: 1-pass coordinate-descent, ranges to-be-justified: (1) init at the **M0-measured** optimizer center (`training_recipe.md` §7; WD/LR reclassified from B01-locked → M0-measured 2026-06-01); (2) order WD → epochs → LR; (3) per-axis ~4–5-point log-spaced bracket whose range a ~1-GPU-h Lite micro-probe sets by locating the loss/AUROC U-turn — do NOT seed near 30×/≈3.0, but **do NOT cap at the bracket top either: a boundary winner means the U-turn isn't bracketed, so escalate WD upward toward Kim's verified ≈3.2 (32× over the 0.1 baseline) until the optimum goes interior** (mirrors the §7 M0 grid's boundary-escalation rule — keep the start grid small, climb only if the edge wins); (4) one round at Lite, re-sweep WD only if the epoch move > 2×; (5) stop at ΔAUROC < 0.02 CrossSession; (6) defer the ensemble-specific (WD, epochs) re-tune to L3 — L1's single-model optimum is NOT L3's; (7) keep mask fixed (B03/B30); mask×WD is P2-if-L1-wins. Also sweep stochastic-depth/DropPath (1603.09382) + attention/FFN dropout in the same descent.
- **Key caveat**: the gate-optimal WD may differ in sign and magnitude from the recon-loss-optimal WD (2602.11137). Sweep WD by gate AUROC; report the recon-loss optimum only as a control. The 30× was fit to i.i.d. LM loss and carries no prior on gate-optimal WD for low-SNR iEEG. **Direction-of-escalation note:** Han (2602.11137) found higher WD *worsens* pretrain loss while *improving* fine-tune transfer — so the gate-optimal WD plausibly sits **≥** the §7 M0 recon-gap optimum, making upward escalation (not downward) the more likely-to-bind direction here. The two sweeps are distinct, not redundant: **§7 M0 ranks WD on the train–val recon gap to find a stable, non-collapsing bring-up center; L1 re-ranks on the gate** — expect L1's WD optimum ≥ M0's, and let the boundary-escalation rule carry it up if the gate keeps improving at the edge.
...
**Cross-subject transfer (input-side + latent-space)**
- 1808.05464 — He & Wu, *Transfer Learning for BCI: A Euclidean Space Data Alignment Approach* (IEEE TBME 2020). Unsupervised per-subject covariance whitening (EA protocol). → **L0.5.**
- 1511.05547 — Sun, Feng & Saenko, *Return of Frustratingly Easy Domain Adaptation* (AAAI 2016). Linear CORAL — feature-space second-order alignment. → **L0.5 (latent).**
- 2206.01323 — Kobler et al. (NeurIPS 2022). SPDDSMBN/TSMNet — per-domain SPD-manifold batch-norm (whiten per-subject covariance in feature space). → **L0.5 (latent).**
- 1804.08450 — Huang et al., *Decorrelated Batch Normalization* (CVPR 2018). ZCA-whitening layer. → **L0.5 (latent, 3a).**
...
- 2026-06-03 — L0.5 reframed input-EA → **test-time latent whitening** (3b, frozen-encoder, post-PMA); raw-channel EA ruled out for iEEG (no montage correspondence). Added latent-space family CORAL 1511.05547 / SPDDSMBN 2206.01323 / Decorrelated-BN 1804.08450 (**zero iEEG-latent precedent**); pointer → `ablations.md` §10 `R-whiten-latent` / `R-whiten-global`. EA-revisit 2502.09203 confirmed real.
...
- 2026-06-03 — L1: added the **WD boundary-escalation rule** (start grid small, climb toward Kim's WebFetch-verified ≈3.2 = 32× over 0.1 baseline only if the edge wins) mirroring the new `training_recipe.md` §7 M0 rule; L1 init re-pointed from "B01-locked" → M0-measured optimizer center (WD/LR reclassified to M0-measured 2026-06-01). Added direction-of-escalation note: per Han 2602.11137 (higher WD → worse pretrain loss, better transfer) the gate-optimal WD plausibly sits ≥ the §7 M0 recon-gap optimum, so upward is the likely-to-bind direction; §7 M0 (recon-gap, bring-up center) and L1 (gate, transfer optimum) are distinct sweeps.
- 2026-06-05 — L0: folded in the **step-budget axis** (Ben-directed) after the gate-job P1 curve showed ~94% of the recon-L1 drop by step ~600. Split per "the one rule": gate-scored production budget = L0 (cheap intra-run checkpoint grid + transfer probe, watch the 2503.19206 overtraining U-turn) with a cosine-horizon caveat (intermediate checkpoints under-annealed → purpose-built cosine-to-K confirm = L1 epochs axis); recon-side sweep-truncation = `training_recipe.md §7 M0 step 4` (extended same day to mark the recon plateau as the bring-up/truncation budget, a lower bound on — not equal to — the gate-optimal ceiling).
