...
> **STATUS 2026-06-04 — EXECUTED; historical build plan.** B36 + FE-RAW-1 + P1/P2 paradigm-B own-predictors are LANDED on the executed path (capstone P1→P2→P3→P4 fast-dev-run green; see the `v14_blockers.md` 2026-06-03/06-04 block). The "State of the world / B36 is doc-only / every B36 surface unwired" snapshot below is the **pre-implementation** state — stale, kept for the audit trail. **Code is now ground truth**; the canonical lock index is `MEMORY.md §Status`. The line-5 trust order (which ranks code LAST) held only while B36 was unbuilt — it is now **INVERTED**: for any "is X wired?" question, read the code FIRST.
...
**Trust order on conflict** (⚠️ INVERTED 2026-06-04 — B36 is now built, so **code is ground truth and ranks FIRST**; the ordering below applied only while B36 was doc-only): `memory/project_v14_b36_*` (newest lock) → this doc → `training_recipe.md` → `v14_blockers.md` → `plan.md` → code. Supersedes `docs/neuroprobe/v14_implementation_fix_list.md` (pre-B36, stale).
...
| D8 | RoPE downward generalization (train T_p=40 / read T_p=8) **accepted** as a non-issue (RoPE is relative). Capstone asserts it. |
...
- **C1** Thread a phase-conditional `clip_len` into `build_v14_experiment` → segmenter duration (**5.0s** P1/2/3 → T_p=40; **1.0s** P4 → T_p=8 @ 8 Hz latent). · `experiments/dispatch_v14.py` · **TEST**: segmenter emits T_bin=80→T_p=40 at 5s (16 Hz front-end), T_bin=16→T_p=8 at 1s.
...
**Fixture**: 2 clips, ~8 electrodes over a few DK parcels (K=80 slots, a few covered), Multi-STFT F=50/F_p=10, **5 s** clips for P1/P2/P3 (T_bin=80→T_p=40, uniform under the hop=128 re-lock), and **1 s** for P4 (T_bin=16→T_p=8), d=256, encoder depth 2 (speed), M=1. A 2-clip synthetic Whisper cache (`mean_all`, 250 frames @ 50 Hz → 40 @ 8 Hz) with `fit_channel_stats` over those 2 clips for the P3 `TargetStandardizer`. Lightning `fast_dev_run=1` per phase; each phase loads the prior phase's ckpt via the WS-E driver.

> **P3 `T_p` reconcile (flagged by the WS-I audit, 2026-06-03; RESOLVED by the hop=128 re-lock, 2026-06-03).** This table originally said "P1/P2/P3 at T_p=20" while the live Whisper teacher pool (F2) hard-pins **40** frames (8 Hz × 5 s) and `V14Phase3DistillModule` asserts student `T_p == teacher T_p` — so under hop=256 P3 alone had to run at `T_bin=80 → T_p=40`, diverging from P1/P2. The hop=128 re-lock makes the front-end 16 Hz / latent 8 Hz, so **all of P1/P2/P3 are now uniformly T_p=40 at 5 s** (T_bin=80) and the divergence is gone — the teacher still pins 40 frames and the front-end now matches it natively. Cross-phase encoder load remains clean (RoPE `persistent=False`, size-agnostic conv stem).
