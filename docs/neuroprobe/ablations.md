...
**B36 amendment (2026-06-01, TOP OF STACK — wins on conflict; `memory/project_v14_b36_perparcel_pool_structured_jepa_2026_06_01.md`).** B36 throws away the Perceiver soft-routing for a hard per-parcel attention pool and makes SSL a paradigm-B parcel×time structured JEPA (visible-only encoder + separate narrow transformer predictor, EMA teacher full-input, L1 on post-`encoder_ln` M4), with P1/P2 RE-SPLIT (P1 front-end-only all corpora / P2 pool+inter-parcel+predictor anatomy-only @ front-end LR/10). This **supersedes** the §4 self-distill loss surface (B31 inert 2-term), the §4b B29 joint-phase default, and the §3/§4 Perceiver-IO + soft `log(support+ε)` cross-attn + anatomy-bias-warmup framing. New B36 sisters live in **§4c**. B33 (§8 Phase-3 project-up) and B35 (§10 Phase-4 readout) are untouched — B36 changes the encoder/SSL/staging; P3/P4 read a frozen encoder of unchanged output shape. Cells in §4/§4b/§6 that test the retired soft-routing / 4-term-loss / joint-phase machinery are superseded by their B36 counterparts in §4c; read §4c first where they conflict.
...
## 4c. Stage-2 SSL — B36 paradigm-B parcel×time masked JEPA (post-B36 lock, supersedes §4 loss surface + §4b joint default)

B36 (2026-06-01) replaces the §4 self-distill loss surface and the §4b joint phase. **Default**: hard per-parcel attention pool (query k reads only parcel-k electrodes, block-diagonal one-hot mask) → inter-parcel self-attn encoder (M4) → **paradigm-B** masked JEPA: visible parcel cells through the encoder, a separate narrow transformer **predictor** (3 blocks @ d=128, 4 heads, MLP 4×, ~0.6M, discarded at deploy) predicts EMA-teacher targets at masked (parcel × contiguous-time-block) cells (~30% of covered), targets = teacher's **post-`encoder_ln` M4** (stop-grad, no separate target LN — canonical V-JEPA), **L1** over masked cells only. Masked parcels' electrodes are dropped UPSTREAM at the (electrode, time-patch) input level (leakage-free). EMA τ=0.99925 fixed (B27). Staged: P1 front-end-only @ M2 (all corpora incl. SWEC) / P2 pool+inter-parcel+predictor @ M4 (anatomy-only, front-end LR/10, no M2-aux). RankMe / effective-rank logged from step 0 (low-cardinality collapse tripwire).

| Cell | Tests | Priority |
|---|---|---|
| **R-no-bottleneck** | **= BaRISTA's verified architecture** (channel tokens + additive Destrieux-parcel embed + region masking, NO pool/compression). The thesis-defining falsifier — if the pooling bottleneck + inter-parcel SA doesn't beat this, v14 is the FM-anchor paper, not the anatomy-bottleneck paper. Additive embed is strictly more expressive; the pool only pays off cross-subject. **Run under LOSO** (where the cross-subject prior is testable), not just Neuroprobe pairwise. Nearly free (config of the same code). | **P0 MANDATORY** |
| **R-no-whisper** | JEPA-only, no Whisper FM anchor, on the cross-subject track. Separates "anatomy bottleneck" transfer from "FM anchor" transfer (confounded otherwise). | **P0 MANDATORY** |
| **R-joint-ssl** | Restores the B29 joint P1+P2 phase (single SSL phase, no staged seam) at matched step budget. Falsifies the B36 P1/P2 re-split. | **P0** (B36 §7) |
| **R-paradigm-a-mlp** | Paradigm A: mask tokens through the inter-parcel encoder + BaRISTA-faithful 5-layer FC MLP predictor (no separate visible-only encoder). Cheap first-class sister — if it ties the frozen-probe number, the simpler design wins. | **P0** (B36 §5) |
| **R-predictor-depth-{2,4}** | Predictor block-count sweep around the 3-block default (2 and 4). Cheap, JEPA-sensitive, precedents diverge (V-JEPA ½-depth, Brain-JEPA ½-depth/½-width) — empirical, not an armchair lock. RankMe-monitored. | **P0** (B36 §5) |
| **R-mask-ratio-{15,45}** | Mask-ratio bracket around the 30% default (brain-region regime: BaRISTA 30% spatial, Brain-JEPA ~16% held-out — NOT V-JEPA's ~90% dense-video rate). | P1 (B36 §3) |
| **R-whole-region-mask** | Mask whole covered parcels across all time (drop the contiguous-time-block structure) — tests whether per-time-block masking earns its keep over full-region masking (Brain-JEPA Cross-ROI variant). | P1 (B36 §3) |
| **R-cross-time-predictor** | Predictor attends across time (masked (k,t) reads visible parcels at other time-patches), vs the per-time-patch default. Tests whether cross-time parcel attention adds signal beyond the front-end's in-electrode time mixing. | P1 (B36 §3) |
| **R-coverage-robust-pool** | Intra-parcel coverage robustness: drop a subset of a parcel's electrodes upstream → **BYOL/SimSiam** self-distill the partial-coverage M3 token (post-pool, pre-inter-parcel-SA) onto the EMA full-coverage token via a small **student-branch MLP predictor** (+dropped-electrode-id embed); L1, small drop-fraction. Aimed at the CSubject prong (novel subject = different per-parcel coverage). Replaces the stale B22/B31 `L_mid_slot` `R-add-m3-loss` definition. | **P-deferred** (trigger ↓) |
...
**Direct baseline (not a sister)** — BaRISTA, VERIFIED against 2512.12135v1 (6/01): per-channel tokens + **additive** Destrieux-parcel / DK-lobe / channel-coord embed (**no pool**); 12-layer d=64 RoPE transformer; shallow EMA tokenizer target (5-block dilated CNN, hidden dim 5); paradigm A; 5-layer FC MLP predictor d=64; MSE; EMA 0.996; ~1M params; **BT-only 29.2 h**; selective fine-tune (tokenizer frozen); **no FM anchor**. Numbers (corrected — "0.841 vs 0.795" was a mis-pairing): **Table 1** random-split 0.862/0.869 (sentence-onset/speech) vs PopT 0.795/0.775; **Table 2** parcels 0.862 > lobes 0.842 > channels 0.778; **Table 4 LOSO cross-subject 0.841** sentence-onset (= our cross-subject head-to-head target, NOT 0.862).

**The moat is POOLING, not anatomy.** BaRISTA already uses Destrieux parcels and parcels beat channels (0.862 vs 0.778), so "anatomy-indexed" is not a v14 differentiator. The precise distinction: BaRISTA *adds* a parcel embedding to **uncompressed per-channel tokens**; v14 *pools* electrodes into K parcel tokens. Additive embedding is **strictly more expressive** (a superset — the transformer can emulate a soft pool yet keeps channel resolution); the hard pool is a constrained subclass whose lower capacity is a **feature only cross-subject** (subject-invariant canonical K-token frame → cuts subject-ID leakage, the weak CSubject prong). Tension: our scale advantage argues *for* the expressive additive model (bitter lesson). Empirical — `R-no-bottleneck` **under LOSO** adjudicates; `R-m4-slots` is the expressivity↔invariance dial. v14's verified differentiators over BaRISTA, ranked: **(1) Whisper FM anchor** (none in BaRISTA), **(2) multi-corpus scale** (BaRISTA = BT-only 29.2 h; only visible under LOSO), **(3) pooling bottleneck** (on probation = `R-no-bottleneck`), **(4) contextualized M4 target** (vs BaRISTA's shallow tokenizer). Full reasoning + spec: [[project_v14_b36_perparcel_pool_structured_jepa_2026_06_01]] §10. Paper wording is Ben's (IRONCLAD).

**LOSO cross-subject head-to-head (eval contract — REQUIRED).** Neuroprobe's native CrossSubject = train-ONE / test-one (pairwise; structurally cannot show scale). Add **LOSO = train-(N−1) / test-one** on the 9-subject BT cohort: (a) BaRISTA's exact tasks (sentence-onset, speech/non-speech, binary AUROC) head-to-head vs the LOSO 0.841; (b) Neuroprobe multiclass for gate continuity; (c) **both frozen-probe AND selective-FT arms** (else the eval regime confounds the arch comparison); (d) `R-no-bottleneck` run under the SAME LOSO. Already mandated as **LOPO** in `docs/objectives.md §Evaluation philosophy`; surfaced here against the leaderboard framing.

**Superseded §4/§4b/§6 cells under B36** (their machinery is retired — read the B36 counterpart, do not dispatch the original against the B36 default): the §4 4-term-loss roster (`BiL-Loss-Default/NoUtt/NoPre/NoMid/LambdaUttSweep`, the DKoleo/VICReg `R-dkoleo-*`/`R-vicreg-*` family, `R-context-loss-vjepa21-recipe`, `R-perceiver-*-cross-attns`, `R-anatomy-bias-step/-on-from-p1`) — soft-routing + self-distill machinery deleted; the §4b `R-parcels-supervised-gating` / `R-with-anatomy-step-warmup` (λ_anat gone under hard routing); the §6 `R-p2-parcel-mask` / `R-p2-shaft-tube-full-time` / shaft-mask roster — replaced by the B36 parcel×time mask. EMA-τ sweeps (`R-ema-tau-*`, `R-ema-ramp-v-jepa1`), the front-end §3 F-cells, the §5 sampler cells, and the B30 `latent_valid` derivation carry over unchanged.

Sources: [[project_v14_b36_perparcel_pool_structured_jepa_2026_06_01]] (CANONICAL — §1–§14), [[project_v14_anatomy_gated_symmetric_2026_05_28]] (B30 `latent_valid` extended three-way), [[project_v14_b33_project_up_phase3_2026_05_30]] (P3 untouched), [[project_v14_b35_p4_frozen_pma_mean_linear_2026_05_31]] (P4 untouched).
