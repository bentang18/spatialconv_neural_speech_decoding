...
- **Per-electrode temporal self-attn (3c) — single-cell ablation**: dropped from v14 default 2026-05-13 (commit `b09b55c`) after audit flagged it as an unagreed implementation interpretation of the "RoPE on T_bins" spec. Temporal positional info is now carried by one-sided RoPE on K inside the parcel cross-attn (commit `b09b55c`). The dropped block enabled content-relative-to-context features within an electrode (e.g. "rising edge of a gamma burst") that pure RoPE-on-K cannot easily express. Reintroduce as one Stage-1 cell — `depth_temporal ∈ {0, 1}` sister run — if (a) cold-start sits below 0.611 on multi-class CrossSession, OR (b) Stage-2 SSL JEPA-Level-A reconstruction loss plateaus. Cost: ~200K params at d=128. Decision rule: depth_temporal=1 wins by ≥0.005 on multi-class CrossSession → reintroduce as Stage-2 default; otherwise stays dropped and we cite the ablation in the paper as architectural-minimalism justification.
