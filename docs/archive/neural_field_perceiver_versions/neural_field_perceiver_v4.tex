...
{\normalsize April 2026 \quad\textbar\quad Version 4}
\end{center}

\medskip\noindent\rule{\textwidth}{0.4pt}\medskip

% [Issue 1 of taste: "complete technical reference" → "design document"]
% [Issue 15: soften claim categorization promise]
...
% [Issue 9: "27-way classification" → "three 9-way"]
...
% [Issue 6: define the cross-patient setting more carefully]
...
% [Issue 12: "resolves individual gyral columns" → softer]
...
% [Issue 9: fix classification description]
...
% [Issue 1: acknowledge that equations below are idealizations leading to the implemented model]
...
% [Issue 10: "optimal" → "successful in related settings"]
...
\textbf{Cross-patient supervised transfer works at this data scale.} Our LOPO pipeline achieves PER\,=\,0.762 on held-out S14 (LP-FT regime, grouped-by-token CV), compared to per-patient PER\,=\,0.737 (same CV). Direct evidence that decodable shared structure exists in~$f$.
...
% [Issue 1: acknowledge this is an idealization]
This formulation \textbf{idealizes away patient-specific effects}: $g$ is shared and $\bfz(t)$ is universal. In reality, the observation depends on patient identity through both geometry ($T_i$) and physiology ($r^{(i)}$). The implemented model (Section~\ref{sec:implemented}) restores patient conditioning. Equation~\eqref{eq:obs-function} is the motivating abstraction; the model does not enforce it literally.
...
% [Issue 8: "well-posed" → honest language]
...
% [Issue 13: mark linearized projection as toy abstraction]
...
MNI coordinates provide the geometric relationship between projections a priori, rather than requiring post-hoc alignment (CCA, Procrustes, CORAL---all failed in our experiments).
...
% [Issue 10: "is SfM" → "structurally analogous to"]
Inspired by holistic performance capture~\cite{hewitt2024}, the cross-patient decoding problem maps structurally to \textbf{fitting a parametric 3D model to 2D observations from multiple viewpoints}.
...
% [Issue 10: "Our approach is SfM" → softer]
...
% [Issue 11: add explicit transfer criterion]
...
% [Issue 11: demote prestige markers; Issue 2 of taste: remove "Best Paper", "Oral"]
...
\textbf{VGGT}~\cite{hewitt2024}: joint 3D attribute prediction from variable-length views. Validates Perceiver jointly estimating latent state + alignment.

\textbf{LVSM}~\cite{jin2024}: encoder-decoder with fixed latent tokens vs.\ decoder-only. Decoder-only wins with enough data; explicit structure wins at small scale. \textit{Validates our structured approach for ${\sim}1300$ trials.}
...
% [Issue 3: split confidence gate into static reliability vs dynamic content]
\textbf{Static electrode reliability gate} (for inference-time uncertainty weighting). To propagate position-dependent reliability into the Perceiver attention, we add a per-electrode reliability scalar that depends only on electrode identity---not on current neural activity:
\begin{equation}
c_j^{\text{rel}} = \sigma\Bigl(\text{Linear}\bigl(\text{MNI\_PE}(\tilde{\bfp}_j) + \text{Grid\_PE}(\text{row}_j, \text{col}_j)\bigr)\Bigr) \in (0, 1)
\end{equation}
The reliability gate scales attention \textbf{values} (not logits): $V_j \leftarrow c_j^{\text{rel}} \cdot V_j$ before aggregation.
Because $c_j^{\text{rel}}$ depends only on positional features, it can be legitimately interpreted as ``this electrode position tends to be unreliable''---capturing effects like edge-of-array contact quality, proximity to dead electrodes, or cortical regions with low speech-related activity. It adds $d{+}1 = 65$ trainable parameters.

\assumption{We deliberately exclude current activity from the reliability gate to preserve interpretability. A dynamic content gate $c_j^{\text{dyn}} = \sigma(\text{Linear}(\text{token}_j))$ could learn richer routing but can no longer be interpreted as electrode quality---it may suppress rare but real speech-related activity. If needed, a dynamic gate can be added as a separate mechanism (ablation A7b), but the default is position-only.}
...
% [Issue 3 of taste: "clinical value" → "potential interpretability value"]
\textbf{Conformal prediction}~\cite{angelopoulos2021}. Post-hoc prediction \textbf{sets} with coverage guarantees. Use CV fold as calibration. Potential interpretability value: ``model thinks /b/ or /p/ (both labial stops)''---the confusion structure itself is informative.
...
\node[uq] (uq1) at (5.8, 0) {Static gate $c_j^{\text{rel}}$ scales $V_j$};

\node[box, fill=blue!8] (perceiver) at (0, -1.5) {Perceiver cross-attention (reliability-weighted)};
...
\noindent Beta-NLL affects training gradients (left). Static reliability gate $c_j^{\text{rel}}$ affects inference-time attention (right). MC Dropout captures model uncertainty. Conformal prediction gives calibrated prediction sets. Each mechanism has a distinct scope---they are complementary, not redundant.
...
% [Issue 1: add "The Implemented Model" section making patient conditioning explicit]
\section{The Implemented Model}\label{sec:implemented}

The idealized equations in Part~I motivate the architecture but are not what the model computes. The implemented model is:
\begin{equation}\label{eq:implemented}
\boxed{\hat{d}_j^{(i)}(t) = g_\theta\bigl(\tilde{\bfp}_j^{(i)},\; s^{(i)}(t),\; e^{(i)}\bigr) + \epsilon_j^{(i)}(t)}
\end{equation}
where:
\begin{itemize}[leftmargin=2em]
\item $\tilde{\bfp}_j^{(i)} = R(\omega^{(i)}) \cdot \bfp_j^{(i)} + \Delta^{(i)}$ is the geometrically corrected electrode position.
...
\section{Spatial Encoder (\texorpdfstring{${\sim}$100K}{~100K} params)}
...
Attention values are scaled by the static reliability gate (Section~\ref{sec:uncertainty}):
\begin{equation}
\text{out}_i = \sum_j \text{softmax}(A)_{ij} \cdot \bigl(c_j^{\text{rel}} \cdot V_j\bigr)
\end{equation}
where $c_j^{\text{rel}} = \sigma(\text{Linear}(\text{MNI\_PE}(\tilde{\bfp}_j) + \text{Grid\_PE}(\text{row}_j, \text{col}_j)))$. Electrodes at unreliable positions contribute less.
...
% [Issue 2: Fix decoder contradiction — use position-weighted aggregation, not mean]
...
% [Issue 2: The key fix — each electrode gets a spatially-local view of virtual electrode outputs]
...
Instead, each electrode $j$ aggregates virtual electrode outputs weighted by spatial proximity:
\begin{equation}\label{eq:local-agg}
s_j^*(t) = \sum_{\ell=1}^{L} a_{j\ell} \cdot \text{out}_\ell(t) \in \R^d, \qquad
a_{j\ell} = \frac{\exp\bigl(-\alpha_{\text{dec}} \cdot \|\tilde{\bfp}_j - \text{v\_pos}_\ell\|^2\bigr)}
             {\sum_{m=1}^L \exp\bigl(-\alpha_{\text{dec}} \cdot \|\tilde{\bfp}_j - \text{v\_pos}_m\|^2\bigr)}
\end{equation}
where $\alpha_{\text{dec}} > 0$ is a learnable spatial scale (initialized to match the encoder's $\alpha$).
...
Global readout: $z_{\text{global}} = \text{mean}(\text{all tokens}) \in \R^d$ for $k$-NN.\\
Per-position: $z_p = \text{mean}(\text{tokens in position window}) \in \R^d$ for classification.
...
\textbf{Dual head}:
\begin{equation}
\text{logits}_p = \frac{1}{2}\bigl(\underbrace{\text{Linear}(\bfz \to 9)}_{\text{CE head}} + \underbrace{\tau \cdot \text{normalize}(\text{Linear}(\bfz \to 15)) \cdot A^\top}_{\text{articulatory head}}\bigr)
\end{equation}
...
\node[box, fill=blue!8] (spatial) at (0, 0) {\textbf{Spatial Encoder} (${\sim}$100K)\\Perceiver + dual PE + $\Delta, \omega, e$ + $c_j^{\text{rel}}$};
...
% [Issue 7: separate strict LOPO from transductive SSL]
...
% [Issue 6: define three evaluation regimes explicitly]
...
\begin{enumerate}[leftmargin=2em]
\item \textbf{Zero-shot transfer.} Train on sources (Phase 1). Evaluate on target with no adaptation. Claim: ``the shared model generalizes to unseen electrode configurations.'' This is the strongest scientific claim but likely yields poor PER at our data scale.
...
% [Issue 4: clarify "best-of per fold"]
...
A7 & Uncertainty stack & a.\ Beta-NLL + static gate vs.\ MSE-only\\
 & & b.\ Static gate vs.\ dynamic gate \\
...
\begin{itemize}[leftmargin=2em]
\item Mean PER across all patients improves over current LOPO baseline with best architecture (A1c/d vs.\ A1a), evaluated across ${\geq}3$ seeds, in the LP-FT regime.
\item This is the only criterion that provides scientific evidence for the MNI coordinate hypothesis.
\end{itemize}

% [Issue 5: make S14 explicitly exploratory only, remove threshold language]
...
% [Issue 14: honest about geometric parameters being weak proxies]
...
\item Reliability gate $c_j^{\text{rel}}$ correlates with electrode impedance or SNR---interpretable because it depends only on position, not current activity.
\end{itemize}
...
S14 HGA & 153 trials & Target eval (pilot) & -- \\
...
% [Issue 4 of taste: avoid elegance as surrogate for evidence]
...
The implemented model (Eq.~\ref{eq:implemented}) is honestly patient-conditioned: it conditions on patient identity $e^{(i)}$ and learned geometric corrections $\Delta^{(i)}, \omega^{(i)}$, relaxing the idealized shared observation function. The architecture produces two distinct representations: $s(t)$ (per-frame spatial summary, trained via reprojection) and $\bfz(t)$ (temporally-refined state, trained via classification). The observation decoder uses position-weighted aggregation of virtual electrode outputs (Eq.~\ref{eq:local-agg}), preserving the spatial decomposition that motivates virtual electrodes. It employs a bilinear factorization that assumes separability between spatial tuning and articulatory activation---a structural hypothesis requiring empirical validation (ablation A9).

Key improvements over v3: (1)~implemented model stated as the primary equation, with idealizations clearly labeled; (2)~position-weighted latent aggregation in the decoder resolving the mean-pooling contradiction; (3)~static reliability gate (position-only) separated from dynamic content gating; (4)~three evaluation regimes defined (zero-shot, LP-FT, transductive); (5)~Phase~3 explicitly flagged as transductive; (6)~checkpoint selection clarified as inner-validation, not test-set leakage; (7)~S14 demoted to exploratory pilot evidence; (8)~overclaims reduced throughout; (9)~bibliography added; (10)~manifold ``well-posed'' claim softened; (11)~``27-way classification'' corrected to three 9-way; (12)~linearized projection model marked as toy abstraction; (13)~geometric correction interpretation caveated; (14)~transfer criterion for 3D vision patterns made explicit.
...
\bibitem{singh2025}
Singh, S., et al.\ (2025).
Cross-patient speech decoding from sEEG with per-patient input layers and shared temporal backbone.
\textit{bioRxiv}.
% Group training N=25, per-patient Conv1D + shared BiLSTM 2×64

\bibitem{willett2023}
Willett, F.R., et al.\ (2023).
A high-performance speech neuroprosthesis.
\textit{Nature}, 620, 1031--1036.
% Per-day affine layer + shared 5-layer GRU-512

\bibitem{metzger2023}
Metzger, S.L., et al.\ (2023).
A high-performance neuroprosthesis for speech decoding and avatar control.
\textit{Nature}, 620, 1037--1046.
% Articulatory somatotopy in vSMC, N=1 chronic

\bibitem{wu2025}
Wu, J., et al.\ (2025).
Articulatory feature reconstruction from ECoG.
\textit{Preprint}.
% Speaker-invariant articulatory features from vSMC, PCC 0.75--0.80

\bibitem{chen2025}
Chen, Y.-C., et al.\ (2025).
SwinTW: Cross-patient ECoG speech decoding with shifted-window transformers and MNI coordinates.
\textit{Preprint}.
% 43 ECoG + 9 sEEG, MNI + ROI embeddings, PCC 0.765

\bibitem{safaie2023}
Safaie, M., et al.\ (2023).
Preserved neural dynamics across animals performing similar behaviour.
\textit{Nature Neuroscience}, 26, 714--724.
% Cross-individual motor cortex manifold similarity
...
\textit{ICLR 2024}.
% Factorized spatiotemporal basis from sparse sensors

\bibitem{truong2023}
Truong, P., et al.\ (2023).
SPARF: Neural Radiance Fields from Sparse and Noisy Poses.
\textit{CVPR 2023}.
% NeRF from sparse noisy poses

\bibitem{jha2023}
Jha, A.H., et al.\ (2023).
Senseiver: Perceiver IO for sparse sensor field reconstruction.
\textit{Nature Machine Intelligence}.
% Perceiver IO for sparse sensor reconstruction
