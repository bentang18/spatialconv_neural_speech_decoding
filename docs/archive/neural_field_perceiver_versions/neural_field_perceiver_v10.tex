...
{\normalsize April 2026 \quad\textbar\quad Version 10}
\end{center}
...
\end{equation}
with $L{=}16$ virtual electrodes and $\tilde{\bfp}_j^{(i)}$ the corrected MNI position. The observation decoder then predicts each electrode's activity from its local latent:
\begin{equation}\label{eq:implemented}
\boxed{\hat{d}_j^{(i)}(t) = g_\theta\bigl(\tilde{\bfp}_j^{(i)},\; s_j^{*,(i)}(t),\; e^{(i)}\bigr)}
\end{equation}
where $s_j^{*,(i)}(t) = \sum_\ell a_{j\ell} \cdot s_\ell^{(i)}(t)$ is a position-weighted aggregation of virtual electrode outputs at electrode $j$'s location (Section~\ref{sec:obs-decoder}), and $e^{(i)}$ is a patient embedding.

\textbf{Structural identifiability.} The encoder sees \textbf{no patient identity} --- only electrode positions and activity. The patient embedding $e^{(i)}$ enters only the observation decoder, where it handles patient-specific signal characteristics (impedance, gain, tissue properties). This is a deliberate architectural constraint: by withholding patient identity from the encoder, the latent $s^{(i)}(t)$ is forced to represent the neural field in a patient-agnostic frame, encoding spatial structure through MNI coordinates rather than arbitrary per-patient codes.

\textcolor{BrickRed}{\textit{Residual leakage:}} Even without $e^{(i)}$, the encoder implicitly identifies patients through their unique electrode position sets. This is inherent to any coordinate-aware architecture and is the \textit{intended} behavior --- position-dependent processing is the whole point. The constraint prevents only \textit{non-geometric} patient-specific processing. Whether this is sufficient for practical patient-invariance is tested by ablation A3 (comparing with $e$ in the encoder) and A5 (reprojection weight).

The losses create a gradient tension on the \textit{decoder}:
$\mathcal{L}_{\text{reproj}}$ (with $e^{(i)}$) rewards patient-specific reconstruction;
$\mathcal{L}_{\text{cross}}$ (with $e{=}\mathbf{0}$) rewards patient-invariant spatial structure;
$\mathcal{L}_{\text{class}}$ rewards whatever predicts phonemes.
...
\begin{equation}
\text{token}_j = \underbrace{\text{Linear}(d_j \to d)}_{\text{activity}} + \underbrace{\text{Linear}\bigl(\gamma(\tilde{\bfp}_j) \to d\bigr)}_{\text{Fourier MNI PE}}
\end{equation}
where $\tilde{\bfp}_j = R(\omega^{(i)}) \cdot \bfp_j + \Delta^{(i)}$ is the corrected MNI position. \textbf{No patient embedding enters the encoder} --- patient identity is withheld to enforce structurally patient-agnostic representations (Section~\ref{sec:architecture}).
...
\footnote{NeRF uses $F{=}10$ for sub-pixel detail. We use $F{=}4$ because (a) ${\sim}200$ electrode positions across ${\sim}20$ patients cannot constrain higher frequencies, and (b) the finest wavelength ($3.75$\,mm) already approaches electrode spacing (${\sim}2$\,mm) and inter-subject correspondence uncertainty (${\sim}5$\,mm). The $\ell{=}3$ frequency is useful \textit{within-patient} (where MNI is precise for rigid arrays) but unreliable for \textit{cross-patient} alignment at that scale. Ablation A12 tests $F{=}3$ (dropping the finest frequency).}

\textbf{Per-patient encoder parameters} (6 per patient):
\begin{itemize}[leftmargin=2em]
...
\end{itemize}
These are purely geometric --- they adjust electrode positions, not processing. The patient embedding $e^{(i)} \in \R^d$ (init ${\sim}\,\mathcal{N}(0, 0.02)$) enters only the observation decoder (Section~\ref{sec:obs-decoder}), where $\phi(\tilde{\bfp}, e)$ can learn position-dependent patient corrections.

\textcolor{BrickRed}{\textit{Residual risk:}} $\Delta^{(i)}$ and $\omega^{(i)}$ could in principle serve as arbitrary per-patient codes rather than geometric corrections. Regularization (L2 on $\Delta, \omega$; small init) limits this channel, and 6 scalar parameters are a narrow information bottleneck compared to the $d$-dimensional $e$. Ablation A3 tests the contribution of each per-patient parameter.}
...
\end{equation}
where $\tau_h > 0$ is a learnable temperature per head (init $\sqrt{d_h}$, clamped $\leq 100$), $\bar{\bfp}$ denotes standardized MNI coordinates, and $\Sigma_i = C_i C_i^\top$ is a per-virtual-electrode covariance ($C_i$: lower-triangular Cholesky factor, 6 params each, init $\alpha I$). Normalizing Q and K to unit vectors keeps the content term bounded, so the distance bias has a consistent effect throughout training regardless of embedding norms --- following SwinV2~\cite{liu2022swinv2} and ViT-22B. Each virtual electrode learns the \textbf{shape} of its spatial receptive field: an elongated lip region, a compact tongue area, etc. 96 + 4 params ($16 \times 6$ covariances $+$ 4 temperatures). 4-head attention.
...
The decoder is the \textbf{only} component that sees patient identity. $\phi(\tilde{\bfp}, e)$ learns position-dependent patient effects --- partially modeling the nonlinear residual $\delta_i(\bfp)$. This asymmetry (patient-agnostic encoder, patient-aware decoder) is the architectural mechanism that enforces identifiability (Section~\ref{sec:architecture}). Ablation A9 tests a full MLP fallback.
...
During multi-patient training, when patients $A$ and $B$ have trials with the same token (e.g., both producing /abe/), the shared observation function should explain patient $B$'s observations from patient $A$'s latent state. Both sides are averaged over the \textbf{full trial}:
\begin{equation}
\mathcal{L}_{\text{cross}} = \frac{1}{N_B} \sum_{j=1}^{N_B} \bigl\| \bar{d}_j^{(B)} - g_\theta(\tilde{\bfp}_j^{(B)},\; \bar{s}_A^{*,(B)},\; \mathbf{0}) \bigr\|^2
\end{equation}
where $\bar{d}_j^{(B)} = \tfrac{1}{T}\sum_{t} d_j^{(B)}(t)$ is patient $B$'s trial-averaged observation, and $\bar{s}_A^{*,(B)} = \tfrac{1}{T}\sum_{t} s_A^{*,(B)}(t)$ aggregates patient $A$'s trial-averaged latent at patient $B$'s electrode positions. \textbf{The patient embedding is set to $\mathbf{0}$} --- the decoder must predict patient $B$'s observations using only spatial position and patient $A$'s latent, with no patient-identity shortcut.

\textcolor{BrickRed}{\textit{$\mathcal{L}_{\text{cross}}$ is a deliberately coarse spatial-alignment signal.}} Trial-level averaging collapses exactly the temporal structure that the encoder's learned queries and multi-scale tokenization are designed to preserve. This is an intentional tradeoff, not an oversight:
\begin{itemize}[leftmargin=2em]
\item $\mathcal{L}_{\text{class}}$ captures \textbf{fine temporal} structure (per-position phoneme classification via learned queries).
\item $\mathcal{L}_{\text{reproj}}$ captures \textbf{fine spatiotemporal} structure (per-frame, per-electrode, within-patient).
\item $\mathcal{L}_{\text{cross}}$ captures \textbf{coarse spatial} alignment (trial-averaged, cross-patient).
\end{itemize}
Full-trial averaging is also a practical necessity: per-phoneme onset times from MFA alignment are unreliable for ${\sim}50$\% of patients~\cite{spalding2025}, which motivated full-trial CTC over per-position classification in our existing pipeline. Using phoneme-window averaging in $\mathcal{L}_{\text{cross}}$ would reintroduce the same unreliable timing.

\textbf{Same-token pairing limitation.} Trials are matched by identical token (e.g., /abe/ $\leftrightarrow$ /abe/). With 52 tokens and multiple patients, this provides ${\sim}O(K^2 \cdot 52)$ pairs from $K$ patients. However, same-token matching means the model can satisfy $\mathcal{L}_{\text{cross}}$ by learning token-level templates without discovering a general spatial field. The encoder's lack of patient identity and the decoder's $e{=}\mathbf{0}$ constraint limit this escape hatch, but do not eliminate it. This is the weakest of the three losses and is expected to serve as a regularizer, not the primary learning signal. $\lambda_2 < \lambda_1$ and anneals aggressively; the $\lambda_2{=}0$ fallback drops it entirely (ablation A5).
...
\subsection{Phoneme Decoder (${\sim}$5K classification + ${\sim}$17K readout)}
...
\midrule
$\Delta^{(i)} + \omega^{(i)}$ (encoder) & 6/pt & Per-patient & Geometric correction \\
...
\begin{enumerate}[leftmargin=2em]
\item \textbf{Zero-shot.} Phase 1 only. $\Delta = \mathbf{0}$, $\omega = \mathbf{0}$, $e = \mathbf{0}$, all frozen. Pure coordinates-only generalization --- no patient-specific parameters at all. Strongest claim, likely worst PER.
...
Cumulative marginal gains, not independent. Total: $-13.5\pp$. All on S14, \textbf{per-patient} training, grouped-by-token CV. Per-fold noise is $\pm 0.05$ PER, so improvements $< 2\pp$ may not replicate. The dual head's $-3.0\pp$ is the marginal effect of adding articulatory + CE on top of the previous recipe.

\subsection{Multi-scale temporal: largest isolated LOPO architectural gain}

In \textbf{LOPO} experiments, stride 3+5+10 gave PER $0.766 \to 0.750$ ($-1.6\pp$), the largest \textit{isolated single-change} gain from any architectural modification (distinct from the cumulative per-patient gains in the table above). However, this is below the $\pm 5\pp$ per-fold noise floor and needs multi-seed replication to confirm. Validated on BiGRU; needs re-validation on transformer.
...
\midrule
A1 & Coordinate-aware spatial encoding & \textbf{Central}: do coordinates help? \\
 & \quad a. Current Conv2d+BiGRU baseline & $\to$ 0.762 (anchor) \\
 & \quad b. Perceiver, \textbf{no coordinate PE} (uniform attn) & $\to$ architecture benefit alone \\
 & \quad c. Perceiver + Fourier MNI PE & $\to$ coordinate benefit (c vs b) \\
 & \quad d. (c) + reprojection loss & $\to$ reprojection benefit (d vs c) \\
 & \quad e. (d) + cross-patient reproj.\ & $\to$ cross-patient benefit (e vs d) \\
...
A3 & Per-patient params & $\Delta$ only / $+\omega$ / $+e_{\text{decoder}}$ / $+e_{\text{encoder}}$ (identifiability test) \\
...
A12 & Positional encoding variants & $\gamma(\bfp)$ vs.\ $\text{Linear}(\bfp \to d)$ vs.\ $F{=}3$ (drop 3.75\,mm) \\
...
\textbf{Primary:} Mean PER across all patients improves over current LOPO baseline by $\geq 3\pp$ ($\geq 3$ seeds, LP-FT regime, paired Wilcoxon per-patient test at $\alpha{=}0.05$). The $3\pp$ threshold is pre-registered because our per-fold noise is $\pm 5\pp$ and per-patient noise across seeds is $\pm 2\pp$; improvements below this cannot be reliably distinguished from measurement error. MNI benefit isolated by paired comparisons: A1c vs.\ A1b (coordinate value, same architecture) and A1d vs.\ A1c (reprojection value).
...
\midrule
MNI coordinates unavailable & High & Fallback to scaled grid positions (A1b tests no-coordinate baseline; A1f tests random positions as sabotage control) \\
MNI alignment too imprecise & High & Ablation A1; $\Delta^{(i)}/\omega^{(i)}$ correct residual misalignment; $F{=}3$ variant (A12) avoids finest frequency \\
