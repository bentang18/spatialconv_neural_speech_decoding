...
19. **Precision-style µECoG adds an important spatial caveat: low-order parcel summaries are a pragmatic approximation, not the full local geometry.** Precision Neuroscience's 1,024-channel, 300--400 µm-pitch surface arrays show that adjacent channels remain incompletely correlated, especially in high gamma, and that decoding benefits from both more area and more density. This means within-region cortical structure is genuinely richer than a single mean activation. However, this does **not** invalidate atlas-grounded low-order summaries for our current regime: cross-patient uECoG still faces 5--15 mm registration error and partial/mismatched parcel coverage, so mean + gradient remains a defensible \emph{identifiable} summary. The correct architectural takeaway is hierarchical: local micro-scale encoder (or subparcel basis) $\rightarrow$ atlas/subparcel tokens $\rightarrow$ shared cross-patient dynamics.

20. **Perceiver cross-attention bottleneck is now consensus across 5+ independent groups.** POYO (512 latents), POYO+ (128), Charmander (32), Brain-OF ARNESS (128), v12 (16 VEs), POSSM (1/chunk). All compress variable neural inputs into fixed-size latent spaces via cross-attention. Model scaling provides no downstream benefit at this data scale (Charmander: 8M≈142M). Architecture matters more than capacity.

21. **Cross-species transfer validated for speech.** POSSM: monkey motor cortex pretraining → human speech decoding (PER 19.80% on Willett data, with multi-input modality). Two-phase training: reconstruction loss (Phase 1) → CTC loss (Phase 2). Encouraging for cross-modality (sEEG→uECoG) transfer in v12.

22. **Functional embeddings can outperform coordinates for neural alignment.** FunctionalMap (Javadzadeh 2025): contrastive-learned 32-dim functional embeddings beat MNI coordinates for masked-region reconstruction in SEEG (p<0.001). Zero per-patient params. Complementary to v12: functional embeddings could augment Fourier PE. But requires expert region labels (impractical for cortical uECOG) and tested only on deep brain nuclei.

23. **Within-modality masking is the most critical SSL component for encoding quality.** NEDS: removing within-modality masking drops encoding 50%. Cross-modal masking helps behavior decoding but less important for neural representation learning. Validates v12's temporal masking focus over multimodal objectives.
