#!/usr/bin/env python3
"""Exp 32: Source Prototype Augmentation (Rank 5 — pure eval change).

Instead of adding raw source embeddings to the k-NN pool (which adds noise),
compute per-(patient, position, class) centroids from source patients and use
those ~81 clean centroids as extra reference points.

Rationale: raw source embeddings are noisy due to domain gap. Averaging within
(patient, position, class) produces a cleaner prototype that represents the
mean neural pattern for that phoneme in that patient's grid space. These
centroids are less noisy and better positioned for nearest-centroid retrieval.
...
SOURCE_CENTROID_WEIGHT = 0.5  # weight for source centroids (vs raw)
...
# ============================================================
# NEW: Source Prototype Augmentation
# ============================================================

def compute_source_centroids(backbone, read_ins, all_data, weight=SOURCE_CENTROID_WEIGHT):
    """Compute per-(patient, position, class) centroids from source patients.

    For each source patient, we compute the mean of L2-normalized embeddings
    for each (position, class) combination. This yields up to
    len(SOURCE_PATIENTS) * N_POSITIONS * N_CLASSES centroids (~81 for 3x9x3).

    Returns:
        centroid_embs: (M, D) tensor of centroid embeddings scaled by weight
        centroid_labels: list of M label lists [p1, p2, p3]
            where only the position corresponding to the centroid is meaningful
            — we replicate the most common label at other positions as a dummy.
    """
    centroid_embs = []
    centroid_labels = []
    ...
    for pid in prepare.SOURCE_PATIENTS:
        ri = read_ins[pid]
        ri.eval(); backbone.eval()
        embs = extract_embeddings(backbone, ri, all_data[pid]["grids"])  # (N_i, D)
        labels = all_data[pid]["labels"]  # list of [p1, p2, p3]

        # L2 normalize per-embedding before centroid averaging
        embs_norm = F.normalize(embs, dim=1)

        for pos in range(prepare.N_POSITIONS):
            for cls in range(1, prepare.N_CLASSES + 1):
                idx = [j for j, l in enumerate(labels) if l[pos] == cls]
                if not idx:
                    continue
                centroid = embs_norm[idx].mean(dim=0)  # (D,)
                # Scale centroid by weight (equivalent to down-weighting source)
                centroid_embs.append(centroid * weight)
                # Build a dummy label: use cls at `pos`, use most frequent class
                # at other positions (we only care about position `pos` for eval)
                dummy_label = []
                for p2 in range(prepare.N_POSITIONS):
                    if p2 == pos:
                        dummy_label.append(cls)
                    else:
                        # Most frequent class at this position among the subset
                        counts = [0] * (prepare.N_CLASSES + 1)
                        for j in idx:
                            counts[labels[j][p2]] += 1
                        dummy_label.append(int(np.argmax(counts[1:]) + 1))
                centroid_labels.append(dummy_label)

    if not centroid_embs:
        return None, None

    return torch.stack(centroid_embs, dim=0), centroid_labels


def proto_src_centroids_predict(train_emb, train_labels, centroid_embs, centroid_labels, val_emb):
    """Prototype prediction augmented with source centroids.

    Uses the combined pool of target training embeddings + source centroids
    (already L2-normalized and weighted) as centroids for nearest-centroid
    classification.
    """
    # Combine: target train embs (normalize) + source centroids (already normalized + weighted)
    train_norm = F.normalize(train_emb, dim=1)
    val_norm = F.normalize(val_emb, dim=1)

    # Build augmented embedding pool
    aug_embs = torch.cat([train_norm, centroid_embs], dim=0)
    aug_labels = list(train_labels) + list(centroid_labels)
    ...
    for i in range(val_norm.shape[0]):
        ...
        for pos in range(prepare.N_POSITIONS):
            ...
            best_cls = 1
            for cls in range(1, prepare.N_CLASSES + 1):
                mask = [j for j, l in enumerate(aug_labels) if l[pos] == cls]
                if not mask:
                    continue
                centroid = aug_embs[mask].mean(dim=0)
                centroid = F.normalize(centroid, dim=0)
                sim = (val_norm[i] * centroid).sum().item()
                if sim > best_sim:
                    best_sim = sim
...
# ============================================================
# STAGE 2 + EVAL (adds source prototype centroids)
# ============================================================
...
def train_eval_fold(backbone, head_init, train_grids, train_labels,
    ...
    # Baseline knn pool: raw source embeddings (same as exp25 baseline)
    train_emb_knn = train_emb
    train_labels_knn = list(train_labels)
    if read_ins is not None and all_data is not None and SOURCE_KNN_WEIGHT > 0:
        ...
        train_emb_knn = torch.cat([train_emb, source_emb * SOURCE_KNN_WEIGHT], dim=0)
        train_labels_knn = list(train_labels) + source_labs

    # k-NN (baseline with raw source embeddings)
    knn_preds = knn_predict(train_emb_knn, train_labels_knn, val_emb)
    ...
    # NEW: Source prototype centroids augmentation
    proto_src_per = knn_per  # fallback
    if read_ins is not None and all_data is not None:
        centroid_embs, centroid_labels = compute_source_centroids(
            backbone_copy, read_ins, all_data, weight=SOURCE_CENTROID_WEIGHT)
        if centroid_embs is not None:
            proto_src_preds = proto_src_centroids_predict(
                train_emb, list(train_labels), centroid_embs, centroid_labels, val_emb)
            proto_src_per = prepare.compute_per(proto_src_preds, val_labels)
        else:
            proto_src_preds = proto_preds
            proto_src_per = proto_per
    else:
        proto_src_preds = proto_preds
        proto_src_per = proto_per
    ...
    methods = {
        ...
        "proto_src_centroids": (proto_src_per, proto_src_preds),
    }
...
if __name__ == "__main__":
    ...
    print(f"CHANGE: Source prototype centroids (per-(patient,pos,class) mean embeddings)")
    print(f"  Source centroid weight: {SOURCE_CENTROID_WEIGHT}")
    print()
    ...
    fold_pers = []
    all_preds, all_refs = [], []
    method_pers = {m: [] for m in ["linear", "knn", "simpleshot", "prototype", "proto_src_centroids"]}
    ...
    for fi, (tr_idx, va_idx) in enumerate(splits):
        ...
        print(f"  Fold {fi+1}/{len(splits)}: PER={per:.4f} "
              ...
              f"psc={methods['proto_src_centroids'][0]:.4f} best={best_method})  ({time.time()-ft0:.1f}s)")
