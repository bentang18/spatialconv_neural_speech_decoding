#!/usr/bin/env python3
"""Exp 39: Per-Patient Gradient Scaling + Curriculum.

Brainstorm Rank 11 — Stage 1 training change only.

Two changes to Stage 1:
  1. Patient loss weighting: each patient's loss is scaled inversely
     proportional to trial count, balancing gradient contribution.
     patient_weight = total_trials / (n_patients * patient_trials)
  2. Curriculum: first 50 epochs train only on same-grid patients
     (S16, S22, S23 — same 8x16 as target S14), then add all patients.

Hypothesis: Starting with same-architecture patients removes confounding
from grid-shape differences. Inverse trial weighting prevents high-trial
patients (S33, S39) from dominating gradients.

Stage 2 and eval unchanged with prototype/knn/simpleshot eval.
...
# Curriculum config
# Same 8x16 grid as target S14: S16, S22, S23 (from prepare.PATIENT_GRIDS)
SAME_GRID_PATIENTS = [
    p for p in prepare.SOURCE_PATIENTS
    if prepare.PATIENT_GRIDS.get(p) == (8, 16)
]
CURRICULUM_EPOCH = 50  # Add other patients after this many epochs
...
# ============================================================
# STAGE 1 — curriculum + per-patient loss weighting
# ============================================================
...
def train_stage1(all_data):
    ...
    # Compute per-patient loss weights (inverse trial count, normalized)
    trial_counts = {pid: len(source_train[pid]) for pid in prepare.SOURCE_PATIENTS}
    total_trials = sum(trial_counts.values())
    n_patients = len(prepare.SOURCE_PATIENTS)
    patient_weights = {
        pid: total_trials / (n_patients * max(count, 1))
        for pid, count in trial_counts.items()
    }
    print(f"  Same-grid patients (curriculum phase 1): {SAME_GRID_PATIENTS}")
    print(f"  Per-patient weights: { {p: f'{w:.3f}' for p, w in patient_weights.items()} }")
    ...
    all_pids = list(prepare.SOURCE_PATIENTS)

    for epoch in range(S1_EPOCHS):
        # Curriculum: phase 1 = same-grid only, phase 2 = all patients
        if epoch < CURRICULUM_EPOCH:
            active_pids = [p for p in all_pids if p in SAME_GRID_PATIENTS]
            if not active_pids:
                # Fallback if SAME_GRID_PATIENTS is empty
                active_pids = all_pids
        else:
            active_pids = all_pids

        backbone.train(); head.train()
        for ri in read_ins.values(): ri.train()
        pids_shuffled = list(active_pids)
        ...
        for pid in pids_shuffled:
            ...
            w = patient_weights[pid]  # inverse trial weight
            for start in range(0, len(tr_idx), S1_BATCH_SIZE):
                ...
                # Scale loss by inverse trial count weight
                loss = compute_loss(logits, y, mixup_labels=mixup_y, mixup_lam=mixup_lam) * w
                loss.backward()
        ...
        # Log curriculum transition
        if epoch == CURRICULUM_EPOCH:
            print(f"  S1 epoch {epoch+1}: CURRICULUM TRANSITION — adding all {len(all_pids)} patients")
        ...
        if (epoch + 1) % S1_EVAL_EVERY == 0:
            ...
            phase = "curriculum" if epoch < CURRICULUM_EPOCH else "full"
            print(f"  S1 epoch {epoch+1} [{phase}]: train_loss={avg_loss:.4f}, val_loss={val_loss:.4f} "
...
if __name__ == "__main__":
    ...
    print(f"CHANGE: Curriculum (same-grid first {CURRICULUM_EPOCH} epochs) + inverse trial weighting")
    print(f"Same-grid patients: {SAME_GRID_PATIENTS}")
    print()

    print("=== Stage 1: Source patient training (curriculum + weighted) ===")
    s1_start = time.time()
