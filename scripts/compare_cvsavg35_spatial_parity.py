#!/usr/bin/env python3
"""Compare the cvs_avg35 atlas-modernized prototype against the current oracle."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--cvsavg-cache-dir", type=Path, default=DEFAULT_CACHE_DIR)
    ...
    target_pial, _faces = nib.freesurfer.io.read_geometry(
        str(args.subjects_dir / args.target_subject / "surf" / "lh.pial")
    )
    target_pial = np.asarray(target_pial, dtype=np.float64)
    ...
    for patient_id in args.patient:
        ...
        cvsavg_cache = load_cvsavg_pial_cache(patient_id, args.cvsavg_cache_dir)
        if oracle_cache.names != cvsavg_cache.names:
            raise ValueError(f"{patient_id}: oracle and cvs_avg35 bridge names differ")
        ...
        prototype_support = sample_baked_support(cvsavg_cache, atlas)
        ...
        for idx, name in enumerate(cvsavg_cache.names):
            comparison_rows.append(
                {
                    "patient_id": patient_id,
                    "electrode": name,
                    "hemisphere": cvsavg_cache.hemisphere[idx],
                    "outer_vertex": int(cvsavg_cache.outer_vertex_ids[idx]),
                    "pial_vertex": int(cvsavg_cache.pial_vertex_ids[idx]),
                    "d_outer_mm": float(cvsavg_cache.outer_snap_mm[idx]),
                    "d_outer_to_pial_mm": float(cvsavg_cache.outer_to_pial_mm[idx]),
        ...
        patient_rows.append(
            {
                "patient_id": patient_id,
                "n_electrodes": len(cvsavg_cache.names),
                "n_valid": int(valid.sum()),
                "argmax_agreement": agreement_rate,
                "tier1_token_support_spearman": spearman,
                "outer_snap_median_mm": float(np.median(cvsavg_cache.outer_snap_mm)),
                "outer_to_pial_median_mm": float(np.median(cvsavg_cache.outer_to_pial_mm)),
            }
        )
        _plot_patient_overlay(
            ...
            valid_mask=valid,
            verts=target_pial,
            out_path=out_dir / "figures" / f"{patient_id}_cvsavg35_overlay.png",
        )
    ...
    _write_csv(
        ...
        [
            ...
            "n_valid",
            "argmax_agreement",
            "tier1_token_support_spearman",
            "outer_snap_median_mm",
            "outer_to_pial_median_mm",
        ],
    )
    _write_csv(
    ...
    decision_lines = [
        "# cvs_avg35 atlas-modernized parity report",
        "",
        "## Cohort rule",
    ...
    for row in patient_rows:
        ...
        decision_lines.append(
            ...
            f"outer_to_pial_median_mm={float(row['outer_to_pial_median_mm']):.3f}"
        )
    decision_lines.extend(
        [
            ...
            "- If this fails badly, the next thing to revisit is the outer->pial bridge itself.",
        ]
    )
