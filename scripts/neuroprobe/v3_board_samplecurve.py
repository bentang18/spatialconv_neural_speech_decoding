...
SEEDS_FOR_N = {16: 5, 32: 5, 64: 5, 128: 5, 256: 3, 512: 3, 1024: 3}
...
def _merge(shard_dir) -> dict:
    pts, census, anchor, tags, contig = [], [], [], set(), set()
    for path in sorted(glob.glob(f"{shard_dir}/wscurve_*.json")):
        with open(path) as f:
            sh = json.load(f)
        tags.add(sh["tag"])
    ...
    return {"points": pts, "census": census, "anchor": anchor,
...
def _report(m) -> None:
    ...
    print("\n" + "=" * 96)
    print(f"R31 LABEL-EFFICIENCY CURVE   tags={m['tags']}  contiguous={m['contiguous']}")
    print(f"units: {len(cells)} sessions x {len(tasks)} tasks = {len(cells) * len(tasks)} "
          f"(board convention is 180; NEVER read below it)")
    print("=" * 96)
    ...
    cs = m["census"]
    for n in sorted({c["n"] for c in cs if c["n"] <= max(N_GRID)}):
    ...
    for col in COLUMNS:
        ...
        if got is None:
            print(f"    enc12 does NOT reach it within N<={max(N_GRID)} ⇒ NO TIER-1 CLAIM at this grid")
        else:
        ...
        for n in sorted(c12, key=_x_order):
            ...
            d = [np.nanmean(pair[(c, t, "enc12_elec")]) - np.nanmean(pair[(c, t, "enc0_elec")])
                 for (c, t) in keys
                 if (c, t, "enc12_elec") in pair and (c, t, "enc0_elec") in pair]
...
def main():
    ...
    p.add_argument("--mode", default="ws", choices=("ws", "merge"))
    p.add_argument("--cache-dir")
    p.add_argument("--tag", default="pbs50_cd45k")
    p.add_argument("--index", type=int, default=0, help="index into LITE_SESSIONS (array task id)")
    p.add_argument("--shard-dir", required=True)
    p.add_argument("--out", default="samplecurve.json")
    p.add_argument("--taps", default=",".join(ELEC_TAPS))
    ...
    session = LITE_SESSIONS[args.index]
    t0 = time.perf_counter()
    print(f"[R31] S{session[0]}T{session[1]} taps={taps} contiguous={args.contiguous} "
          f"N_GRID={N_GRID} seeds={SEEDS_FOR_N}", flush=True)
    sh = _shard(args.cache_dir, args.tag, session, taps, args.contiguous)
    os.makedirs(args.shard_dir, exist_ok=True)
    out = f"{args.shard_dir}/wscurve_{sh['name']}.json"
