"""Submit Stage-0 L.5.P3 reference-id positive-sanity probe on DCC. Single job."""
...
def main() -> None:
    ...
    out_dir = args.out_dir or Path(
        f"reports/neuroprobe_stage0_l5_p3_reference_id_{datetime.now():%Y_%m_%d}"
    )
    out_dir.mkdir(parents=True, exist_ok=True)
    cmd = [
        ".venv/bin/python",
        "scripts/neuroprobe/run_l5_p3_reference_id.py",
