...
from scripts.neuroprobe.r31_two_axes import LEDGER_K, VISUAL, ledger_check, two_axes, verdict
...
def _pts(gap_of, n_subj=6, tasks=("a", "b", "c", "d", "e"), ns=(16, 64, 256, 1024), noise=0.0,
    ...
    Headroom is spread across tasks so a slope is identifiable at all; `gap_of(headroom)` is the
    law being planted. One cell per subject keeps the cell-mean layer a no-op so the test targets
    the fit, not the aggregation, which `_panel` is already tested for.
    """
    ...
    for si in range(n_subj):
        for n in ns:
            for t, h in zip(tasks, heads):
                ...
                for tap, v in (("enc0", base), ("enc12", base + gap_of(h))):
...
def test_planted_pure_multiplier_reads_THROUGH_THE_ORIGIN() -> None:
    r = two_axes(_pts(lambda h: 0.20 * h), "enc0", "enc12", nboot=200)
...
def test_planted_flat_constant_reads_FLAT() -> None:
    ...
    r = two_axes(_pts(lambda h: 0.03), "enc0", "enc12", nboot=200)
...
def test_planted_offset_plus_slope_reads_BOTH() -> None:
    r = two_axes(_pts(lambda h: 0.02 + 0.20 * h, noise=0.004), "enc0", "enc12", nboot=400)
...
def test_dropping_tasks_actually_drops_them() -> None:
    ...
    r = two_axes(_pts(lambda h: 0.20 * h, tasks=tasks), "enc0", "enc12",
