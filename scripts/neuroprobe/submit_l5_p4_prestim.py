"""Submit Stage-0 L.5.P4 pre-stimulus window leakage probe on DCC.

Runs the L.2-winner config but with the feature window placed at [-1, 0]s
relative to word onset (i.e., entirely before any speech production-related
neural activity could plausibly drive the label). Flag if AUROC > chance + 0.05.

If pre-stimulus features can decode the task label substantially above chance,
either (a) the window definition is wrong, (b) the previous-trial response
leaks into this trial's pre-onset window, or (c) some non-neural confound
(blocking structure, exposure pattern) is decodable. CrossSession scope.
...
def main() -> None:
    ...
    out_root = args.out_root or Path(
        f"reports/neuroprobe_stage0_l5_p4_prestim_{datetime.now():%Y_%m_%d}"
    )
    ...
    for sub_id, trial_id in sessions:
        ...
        sbatch_path.write_text(build_sbatch(
            f"l5p4_sub{sub_id}_trial{trial_id}",
...
def build_command(
    ...
    cmd = [
        ...
        "--anchor-start-before", "1.0",
        "--anchor-end-after", "0.0",
        "--cell-id", cell_id,
        "--seed", str(args.seed),
    ]
