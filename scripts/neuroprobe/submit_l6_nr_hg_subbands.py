"""Submit Neuroprobe Stage 0 L.6.NR narrowband HG sub-band sweep on DCC.

Three narrowband HG envelopes (70-90 / 90-120 / 120-150 Hz) at the L.2 winner
reference (shaft_laplacian) and L.1 winner normalization (train_set_fixed).
Tests where in HG the discriminative information sits — single broadband
hg_envelope (70-150 Hz) view bundles three sub-bands; this asks whether one
sub-band carries the load or whether all three contribute.

Spec note: this redefines L.6.NR from the original docs/neuroprobe/stage_0.md
sketch (which described feature-level nuisance regression). The HG sub-band
question is more directly informative for the v14 cross-modal-SSL choice
(D-SigLIP layer match) and the architectural question of whether HG should be
banded or broadband.

L.1.N1 × R4 × narrowband HG, seed=42, 3 sub-bands × 12 BT Lite sessions = 36 jobs.
"""
...
def main() -> None:
    ...
    out_root = args.out_root or Path(
        f"reports/neuroprobe_stage0_l6_nr_hg_subbands_{datetime.now():%Y_%m_%d}"
    )
    out_root.mkdir(parents=True, exist_ok=True)
    sessions = list(BT_LITE_SESSIONS)

    rows: list[dict[str, str]] = []
    for cell_id, view_kind in SUBBANDS:
        for sub_id, trial_id in sessions:
            ...
            sbatch_path.write_text(build_sbatch(
                f"l6nr_{cell_id.split('.')[-1]}_sub{sub_id}_trial{trial_id}",
