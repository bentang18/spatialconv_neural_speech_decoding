"""Submit spectral × normalization grid at CrossSubject on DCC.

Question: what normalization is principled for spectral views at CrossSubject?
Lens: N1 (per feature-cell std) DESTROYS cross-freq shape — the mechanism that
makes spectral beat envelope cross-subject. N8 (per-channel z) PRESERVES shape
and only strips per-channel scale (the subject-identity confound). N2 is the
transductive sanity-check ceiling.

Holds view = log_stft constant (dynamic-range compressed, so per-channel z
should be the principled match — see
[[project_per_channel_scale_within_vs_cross_protocol_2026_05_15]]).

3 refs × 3 norms × 10 BT_LITE sessions (subject 2 excluded as
DS_DM_TRAIN_SUBJECT_ID) = 90 jobs at 64G.

Cell list:
  R1 bipolar  × log_stft × {N1, N2, N8}
  R4 shaft_laplacian × log_stft × {N1, N2, N8}
  R5 shaft_car × log_stft × {N1, N2, N8}

Note: R4 × log_stft × N1 already exists in L.2 ref/view CSubject sweep at
0.6250 binary AUROC; re-running here for self-contained comparison.
"""
...
def _build_cells() -> tuple[Cell, ...]:
    return tuple(
        Cell(f"L.SN.{ref_tag}_log_stft_{norm_tag}", ref_kind, norm)
...
def main() -> None:
    ...
    out_root = args.out_root or Path(
        f"reports/neuroprobe_stage0_sn_spectral_norm_grid_cross_subject_{datetime.now():%Y_%m_%d}"
    )
