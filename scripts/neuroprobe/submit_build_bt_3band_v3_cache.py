#!/usr/bin/env python
"""Submit the v14_converged_v3 3-band (SLOW+MID+HGA) magnitude spec-cache build to
...
60/120/.. → 0.5 Hz HPF → STFT → |mag| → per-(elec,bin,session) robust-z → winsor);
only the STFT band params differ. STATIC bad-electrode exclusion bakes in (the
only cache-bakeable guard, band-independent via anatomy.extra_bad_electrodes);
WINSOR + CLIP stay runtime, NOT baked. The per-session bad-WINDOW (clip) guard is
SEPARATE (sampling-layer --bad-window-dir), handled by precompute_bad_windows.

PARALLELISM: one independent CPU array task per (band x session) — pretrain
3x13 = 39, eval 3x12 = 36. Default partition ``common,scavenger`` so SLURM lands
each task wherever a node is free; ``--requeue`` re-runs any preempted task
(each task independent + idempotent).
...
_get_data flows through bt_load_raw so the STATIC drop lands. pretrain
(--electrode-set all) and eval (--electrode-set lite) bake DIFFERENT per-session
memmaps, so each corpus needs its OWN --spec-cache-dir + --extractor-cache-folder
(run the script once per corpus).

Usage (via scripts/dcc/dispatch so the run is logged) — run PER CORPUS:

    # full pretrain (13 full-montage SSL sessions)
    scripts/dcc/dispatch scripts/neuroprobe/submit_build_bt_3band_v3_cache.py \\
        --corpus pretrain \\
        --spec-cache-dir /work/ht203/cache_neuroai/v14_3band_v3_spec_pretrain \\
        --extractor-cache-folder /work/ht203/cache/v14_extractors_3band_v3_pretrain

    # lite eval (12 leaderboard sessions)
    scripts/dcc/dispatch scripts/neuroprobe/submit_build_bt_3band_v3_cache.py \\
        --corpus eval \\
        --spec-cache-dir /work/ht203/cache_neuroai/v14_3band_v3_spec_lite \\
        --extractor-cache-folder /work/ht203/cache/v14_extractors_3band_v3_lite

Single-band single-session smoke first:

    ... --corpus pretrain --bands hga --array-range 0-0 \\
        --spec-cache-dir /work/ht203/cache_neuroai/v14_3band_v3_smoke \\
        --extractor-cache-folder /work/ht203/cache/v14_extractors_3band_v3_smoke
"""
...
def main() -> None:
    ...
    log_dir = args.log_dir or Path(
        f"reports/bt_3band_v3_cache_build_{datetime.now():%Y_%m_%d}"
    )
    ...
    for corpus in corpora:
        for band in bands:
            ...
            print(f"[sbatch 3band {corpus}/{band}] {res.stdout.strip()}")
    if args.dry_run:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--atlas", default="dkt", choices=("dk", "dkt"),
                   help="Atlas for the support/valid_mask extractors. Cache-irrelevant "
                        "to the per-band spec memmap; default dkt to match the v3 run.")
