#!/usr/bin/env python3
"""Exp 35: Manifold Mixup Post-Read-In (feature-space mixing).

Brainstorm Rank 10 — Stage 1 training change only.

Instead of mixing in input space (exp25 mixup), mix in the d=256 feature
space after the per-patient read-in. Each step:
  1. Sample 2 random patients
  2. Get a mini-batch from each, augment, pass through their respective read-ins
  3. Truncate to same batch size
  4. lam = Beta(0.2, 0.2)
  5. feat_mixed = lam * feat1 + (1-lam) * feat2
  6. loss = lam * CE(head(backbone(feat_mixed)), labels1)
          + (1-lam) * CE(head(backbone(feat_mixed)), labels2)

Hypothesis: feature-space mixing forces the backbone to learn representations
that interpolate meaningfully between patients, improving cross-patient transfer.

Stage 2 and eval unchanged from exp25 (with prototype/knn/simpleshot).
...
LABEL_SMOOTHING = 0.1
FOCAL_GAMMA = 2.0
# NOTE: input-space mixup disabled for S1; manifold mixup replaces it
MANIFOLD_MIXUP_ALPHA = 0.2
# S2 still uses input-space mixup (same as baseline)
MIXUP_ALPHA = 0.2
TTA_COPIES = 16
KNN_K = 10
...
# ============================================================
# STAGE 1 — Manifold Mixup in feature space
# ============================================================
...
def train_stage1(all_data):
    ...
    # Compute total steps for batching consistency
    total_train_samples = sum(len(source_train[p]) for p in pids)
    steps_per_epoch = total_train_samples // S1_BATCH_SIZE
    ...
    for epoch in range(S1_EPOCHS):
        ...
        # Manifold mixup: each step samples 2 patients and mixes in feature space
        for _step in range(max(steps_per_epoch, 1)):
            # Sample 2 distinct patients
            p1, p2 = np.random.choice(pids, size=2, replace=False)

            # Sample mini-batch from patient 1
            tr1 = source_train[p1]
            if len(tr1) < 2: continue
            idx1 = np.random.choice(tr1, size=min(S1_BATCH_SIZE, len(tr1)), replace=False)
            x1 = augment(all_data[p1]["grids"][idx1]).to(DEVICE)
            y1 = [all_data[p1]["labels"][i] for i in idx1]

            # Sample mini-batch from patient 2
            tr2 = source_train[p2]
            if len(tr2) < 2: continue
            idx2 = np.random.choice(tr2, size=min(S1_BATCH_SIZE, len(tr2)), replace=False)
            x2 = augment(all_data[p2]["grids"][idx2]).to(DEVICE)
            y2 = [all_data[p2]["labels"][i] for i in idx2]

            # Pass through respective read-ins to get features
            feat1 = read_ins[p1](x1)  # (B1, d_flat, T)
            feat2 = read_ins[p2](x2)  # (B2, d_flat, T)

            # Truncate to same batch size
            min_B = min(feat1.shape[0], feat2.shape[0])
            feat1 = feat1[:min_B]; feat2 = feat2[:min_B]
            y1 = y1[:min_B]; y2 = y2[:min_B]

            # Manifold mixup in feature space
            lam = float(np.random.beta(MANIFOLD_MIXUP_ALPHA, MANIFOLD_MIXUP_ALPHA))
            feat_mixed = lam * feat1 + (1 - lam) * feat2

            optimizer.zero_grad()
            h = backbone(feat_mixed)
            logits = head(h)
            # Mixed loss: lam * loss1 + (1-lam) * loss2
            loss = compute_loss(logits, y1, mixup_labels=y2, mixup_lam=lam)
            loss.backward()
...
# ============================================================
# STAGE 2 + EVAL (unchanged from exp25)
# ============================================================
...
def train_eval_fold(backbone, head_init, train_grids, train_labels,
    ...
    # Build embeddings for all eval methods
    ...
    # SimpleShot with source embeddings
...
if __name__ == "__main__":
    ...
    print(f"CHANGE: Manifold Mixup in feature space (d=256 after read-in, Beta(0.2,0.2))")
    print()

    print("=== Stage 1: Source patient training (manifold mixup) ===")
    s1_start = time.time()
