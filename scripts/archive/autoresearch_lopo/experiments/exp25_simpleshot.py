#!/usr/bin/env python3
"""Exp 25: SimpleShot evaluation (center + L2 normalize + nearest centroid).

Brainstorm Rank 7 — pure eval change, zero training modification.

SimpleShot (Wang et al. 2019): Gold-standard few-shot baseline.
1. Extract embeddings from frozen backbone
2. Center: subtract mean of training embeddings
3. L2 normalize
4. Nearest centroid: compute per-(class, position) centroids, classify by cosine sim

Also adds prototype eval (Rank 2): per-class centroids without centering.
...
# ============================================================
# EVAL HELPERS (baseline + NEW SimpleShot + Prototype)
# ============================================================
...
def simpleshot_predict(train_emb, train_labels, val_emb):
    ...
    # Compute per-(position, class) centroids
    preds = []
    for i in range(val_norm.shape[0]):
        ...
        for pos in range(prepare.N_POSITIONS):
            ...
            for cls in range(1, prepare.N_CLASSES + 1):
                # Centroid for this class at this position
...
def prototype_predict(train_emb, train_labels, val_emb):
    """Prototype network: nearest centroid without centering (Rank 2)."""
...
# ============================================================
# STAGE 2 + EVAL (adds SimpleShot + Prototype)
# ============================================================
...
def train_eval_fold(backbone, head_init, train_grids, train_labels,
    ...
    # SimpleShot (NEW) — use target-only embeddings for centering
...
if __name__ == "__main__":
    ...
    print(f"CHANGE: SimpleShot + Prototype eval (pure eval change, no training mod)")
    print()
