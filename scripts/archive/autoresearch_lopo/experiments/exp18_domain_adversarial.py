#!/usr/bin/env python3
"""exp18: Domain adversarial training with gradient reversal in Stage 1.

Hypothesis: Stage 1 overfits to patient-specific features (val diverges epoch 5-10).
Gradient reversal on a patient discriminator forces the backbone to learn
patient-INVARIANT representations. Classic domain adaptation (Ganin & Lempitsky 2015).

Changes from baseline (exp13):
  - Added GradientReversalLayer + PatientDiscriminator
  - Stage 1 trains backbone + patient discriminator with reversed gradients
  - Adversarial loss weight λ follows warmup schedule: 0 → 0.1 over first 30 epochs
  - Multi-patient k-NN evaluation (SOURCE_KNN_WEIGHT=0.5) preserved from exp13
"""
...
# Domain adversarial
ADV_LAMBDA_MAX = 0.1   # max adversarial loss weight
ADV_WARMUP = 30        # epochs to ramp up adversarial loss
...
class PatientDiscriminator(nn.Module):
    """Predicts patient ID from backbone features (used with gradient reversal)."""
    def __init__(self, d_in=64, n_patients=9, hidden=32):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(d_in, hidden),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(hidden, n_patients),
        )

    def forward(self, h, alpha=1.0):
        pooled = h.mean(dim=1)  # (B, d_in)
        reversed_pooled = gradient_reversal(pooled, alpha)
        return self.net(reversed_pooled)
...
# ============================================================
# STAGE 1: MULTI-PATIENT SOURCE TRAINING + DOMAIN ADVERSARIAL
# ============================================================
...
def train_stage1(all_data):
    ...
    # Patient ID mapping for discriminator
    pid_to_idx = {pid: i for i, pid in enumerate(prepare.SOURCE_PATIENTS)}
    ...
    discriminator = PatientDiscriminator(
        d_in=backbone.out_dim, n_patients=n_patients
    ).to(DEVICE)
    ...
    for epoch in range(S1_EPOCHS):
        ...
        # Adversarial lambda warmup: 0 → ADV_LAMBDA_MAX over ADV_WARMUP epochs
        adv_progress = min(epoch / max(ADV_WARMUP, 1), 1.0)
        adv_lambda = ADV_LAMBDA_MAX * adv_progress
        # GRL alpha also ramps up
        grl_alpha = adv_progress
        ...
        for pid in pids:
            ...
            for start in range(0, len(tr_idx), S1_BATCH_SIZE):
                ...
                h = backbone(feat)
                logits = head(h)
                task_loss = compute_loss(logits, y, mixup_labels=mixup_y, mixup_lam=mixup_lam)

                # Domain adversarial: predict patient ID with reversed gradients
                patient_targets = torch.full((len(batch_idx),), pid_to_idx[pid],
                                             dtype=torch.long, device=DEVICE)
                patient_logits = discriminator(h, alpha=grl_alpha)
                adv_loss = F.cross_entropy(patient_logits, patient_targets)

                loss = task_loss + adv_lambda * adv_loss
                loss.backward()

                nn.utils.clip_grad_norm_(
                    list(backbone.parameters()) + list(head.parameters()) +
                    readin_params + list(discriminator.parameters()),
                    S1_GRAD_CLIP,
                )
                optimizer.step()
        ...
        if (epoch + 1) % S1_EVAL_EVERY == 0:
            ...
            print(f"  S1 epoch {epoch+1}: train={avg_loss:.4f}, val={val_loss:.4f}, adv={avg_adv:.4f}, λ={adv_lambda:.3f}")
...
# ============================================================
# STAGE 2 + EVAL (same as exp13 baseline)
# ============================================================
...
if __name__ == "__main__":
    ...
    print(f"EXP18: Domain adversarial λ_max={ADV_LAMBDA_MAX}, warmup={ADV_WARMUP} epochs")
    print()

    print("=== Stage 1: Source patient training + domain adversarial ===")
    s1_start = time.time()
