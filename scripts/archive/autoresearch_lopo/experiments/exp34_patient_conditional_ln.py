#!/usr/bin/env python3
"""Exp 34: Patient-Conditional LayerNorm (Rank 3 — architecture change).

Replace the Backbone's standard LayerNorm with a PatientConditionalLN that
has per-patient learned (gamma, beta) affine parameters.

Hypothesis: Standard LayerNorm applies a single shared affine transform
across all patients. Per-patient affine parameters allow the normalization
to adapt to patient-specific statistics (mean firing rates, amplitude scales,
noise floors) without changing the backbone weights. This is lighter than
per-patient batch norm and compatible with variable-length sequences.

In Stage 1: each source patient has its own LN affine parameters.
In Stage 2: a new patient embedding index is allocated for S14 (index =
len(SOURCE_PATIENTS)), and only this new embedding (+ read-in + head) is
updated while the rest of the backbone is fine-tuned with a small LR.
...
# Patient index mapping: source patients in order, S14 gets the next index
SOURCE_PATIENTS_LIST = list(prepare.SOURCE_PATIENTS)  # deterministic order
PATIENT_TO_IDX = {pid: i for i, pid in enumerate(SOURCE_PATIENTS_LIST)}
TARGET_IDX = len(SOURCE_PATIENTS_LIST)  # S14 gets index = n_source
N_PATIENTS_TOTAL = len(SOURCE_PATIENTS_LIST) + 1  # +1 for target
...
# ============================================================
# NEW: Patient-Conditional LayerNorm
# ============================================================

class PatientConditionalLN(nn.Module):
    """LayerNorm with per-patient learned affine (gamma, beta) parameters.

    The base normalization (zero mean, unit variance) is shared.
    Only the scale and shift are patient-specific, initialized to
    ones and zeros respectively (identical to standard LN at init).

    Input:
        x: (B, d_in, T) — feature tensor before temporal conv
        patient_idx: (B,) long tensor — patient indices
    Output:
        (B, d_in, T) — normalized + affine-transformed
    """
    def __init__(self, d_in, n_patients):
        super().__init__()
        self.base_ln = nn.LayerNorm(d_in, elementwise_affine=False)
        self.gamma = nn.Embedding(n_patients, d_in)
        self.beta = nn.Embedding(n_patients, d_in)
        nn.init.ones_(self.gamma.weight)
        nn.init.zeros_(self.beta.weight)

    def forward(self, x, patient_idx):
        # x: (B, d_in, T) — channels-first from SpatialReadIn
        # LayerNorm expects (B, T, d_in) — normalize over d_in
        x_norm = self.base_ln(x.permute(0, 2, 1))  # (B, T, d_in)
        g = self.gamma(patient_idx).unsqueeze(1)    # (B, 1, d_in)
        b = self.beta(patient_idx).unsqueeze(1)     # (B, 1, d_in)
        return (x_norm * g + b).permute(0, 2, 1)   # (B, d_in, T)


class Backbone(nn.Module):
    """Backbone with PatientConditionalLN replacing standard LayerNorm."""
    def __init__(self, d_in=256, d=32, gru_hidden=32, gru_layers=2,
                 stride=10, gru_dropout=0.3, feat_drop_max=0.3,
                 time_mask_min=2, time_mask_max=5, n_patients=N_PATIENTS_TOTAL):
        super().__init__()
        self.ln = PatientConditionalLN(d_in, n_patients)
    ...
    def forward(self, x, patient_idx=None):
        # x: (B, d_in, T) from SpatialReadIn
        if patient_idx is None:
            # Fallback: use patient 0 for all (should not happen in normal usage)
            patient_idx = torch.zeros(x.shape[0], dtype=torch.long, device=x.device)

        # Apply patient-conditional LayerNorm
        x = self.ln(x, patient_idx)  # (B, d_in, T)
...
def extract_embeddings(backbone, readin, grids, patient_idx_val):
    """Extract embeddings passing patient_idx to backbone."""
    backbone.eval(); readin.eval()
    with torch.no_grad():
        x = readin(grids.to(DEVICE))
        pidx = torch.full((grids.shape[0],), patient_idx_val, dtype=torch.long, device=DEVICE)
        h = backbone(x, patient_idx=pidx)
...
# ============================================================
# STAGE 1 (passes patient_idx to backbone)
# ============================================================
...
def train_stage1(all_data):
    ...
    backbone = Backbone(d_in=d_flat, n_patients=N_PATIENTS_TOTAL).to(DEVICE)
    ...
    for epoch in range(S1_EPOCHS):
        ...
        for pid in pids:
            ...
            for start in range(0, len(tr_idx), S1_BATCH_SIZE):
                ...
                patient_idx = torch.full((len(batch_idx),), pidx_val, dtype=torch.long, device=DEVICE)
                ...
                optimizer.zero_grad()
                feat = read_ins[pid](x)
                h = backbone(feat, patient_idx=patient_idx)
                logits = head(h)
        ...
        if (epoch + 1) % S1_EVAL_EVERY == 0:
            ...
            with torch.no_grad():
                for pid in prepare.SOURCE_PATIENTS:
                    ...
                    pidx_val = PATIENT_TO_IDX[pid]
                    patient_idx = torch.full((len(vi),), pidx_val, dtype=torch.long, device=DEVICE)
                    feat = read_ins[pid](x)
                    h = backbone(feat, patient_idx=patient_idx)
                    logits = head(h)
...
# ============================================================
# STAGE 2 + EVAL (allocates TARGET_IDX for S14)
# ============================================================
...
def train_eval_fold(backbone, head_init, train_grids, train_labels,
    ...
    # In S2, we pass TARGET_IDX for S14 — the new patient embedding
    # will be updated (it starts as ones/zeros = standard LN, same as baseline init)
    optimizer = AdamW([
    ...
    for epoch in range(S2_EPOCHS):
        ...
        for start in range(0, n_train, S2_BATCH_SIZE):
            ...
            patient_idx = torch.full((len(idx),), TARGET_IDX, dtype=torch.long, device=DEVICE)
            mixup_y, mixup_lam = None, 1.0
            ...
            optimizer.zero_grad()
            feat = target_ri(x)
            h = backbone_copy(feat, patient_idx=patient_idx)
            logits = head(h)
            loss = compute_loss(logits, y, mixup_labels=mixup_y, mixup_lam=mixup_lam)
        ...
        if (epoch + 1) % S2_EVAL_EVERY == 0:
            ...
            with torch.no_grad():
                val_pidx = torch.full((len(val_grids),), TARGET_IDX, dtype=torch.long, device=DEVICE)
                feat = target_ri(val_grids.to(DEVICE))
                h = backbone_copy(feat, patient_idx=val_pidx)
                logits = head(h)
    ...
    # Linear predictions with TTA — pass TARGET_IDX for all target inference
    with torch.no_grad():
        if TTA_COPIES > 1:
            logits_sum = torch.zeros(len(val_grids), prepare.N_POSITIONS, prepare.N_CLASSES, device=DEVICE)
            val_pidx = torch.full((len(val_grids),), TARGET_IDX, dtype=torch.long, device=DEVICE)
            feat = target_ri(val_grids.to(DEVICE))
            logits_sum += head(backbone_copy(feat, patient_idx=val_pidx))
            for _ in range(TTA_COPIES - 1):
                aug_grids = augment(val_grids)
                val_pidx2 = torch.full((len(aug_grids),), TARGET_IDX, dtype=torch.long, device=DEVICE)
                feat = target_ri(aug_grids.to(DEVICE))
                logits_sum += head(backbone_copy(feat, patient_idx=val_pidx2))
            logits = logits_sum / TTA_COPIES
        else:
            val_pidx = torch.full((len(val_grids),), TARGET_IDX, dtype=torch.long, device=DEVICE)
            feat = target_ri(val_grids.to(DEVICE))
            logits = head(backbone_copy(feat, patient_idx=val_pidx))
    linear_preds = decode(logits)
    linear_per = prepare.compute_per(linear_preds, val_labels)

    # Build embeddings for eval methods
    train_emb = extract_embeddings(backbone_copy, target_ri, train_grids, TARGET_IDX)
    ...
    if read_ins is not None and all_data is not None and SOURCE_KNN_WEIGHT > 0:
        ...
        for pid in prepare.SOURCE_PATIENTS:
            ...
            emb = extract_embeddings(backbone_copy, ri, all_data[pid]["grids"], PATIENT_TO_IDX[pid])
    ...
    if TTA_COPIES > 1:
        val_emb_sum = extract_embeddings(backbone_copy, target_ri, val_grids, TARGET_IDX)
        for _ in range(TTA_COPIES - 1):
            val_emb_sum = val_emb_sum + extract_embeddings(
                backbone_copy, target_ri, augment(val_grids), TARGET_IDX)
        val_emb = val_emb_sum / TTA_COPIES
    else:
        val_emb = extract_embeddings(backbone_copy, target_ri, val_grids, TARGET_IDX)
...
if __name__ == "__main__":
    ...
    print(f"CHANGE: PatientConditionalLN (per-patient gamma/beta affine, n_patients={N_PATIENTS_TOTAL})")
    print(f"  Patient index mapping: {PATIENT_TO_IDX}")
    print(f"  Target patient index: {TARGET_IDX}")
    print()
