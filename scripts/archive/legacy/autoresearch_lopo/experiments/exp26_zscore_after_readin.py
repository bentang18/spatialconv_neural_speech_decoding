#!/usr/bin/env python3
"""Exp 26: Per-batch z-score normalization after read-in.

Brainstorm Rank 5 — strips patient-specific amplitude/offset from features.

After read-in projects each patient to d=256, apply per-batch z-score:
  feat = (feat - feat.mean(dim=[0,2], keepdim=True)) / (feat.std(dim=[0,2], keepdim=True) + 1e-5)

This forces the backbone to see standardized inputs, preventing it from
using patient-specific feature distributions as shortcuts.

Applied in BOTH Stage 1 and Stage 2.
...
def zscore_features(feat: torch.Tensor) -> torch.Tensor:
    """Per-batch z-score on read-in features. feat: (B, D, T)."""
    # Normalize across batch and time, per feature dimension
    mean = feat.mean(dim=(0, 2), keepdim=True)  # (1, D, 1)
    std = feat.std(dim=(0, 2), keepdim=True) + 1e-5  # (1, D, 1)
...
# ============================================================
# STAGE 1 (with z-score after read-in)
# ============================================================
...
def train_stage1(all_data):
    ...
    for epoch in range(S1_EPOCHS):
        ...
        for pid in pids:
            grids = all_data[pid]["grids"]; labels_list = all_data[pid]["labels"]
            ...
            for start in range(0, len(tr_idx), S1_BATCH_SIZE):
                ...
                optimizer.zero_grad()
                feat = read_ins[pid](x)
                feat = zscore_features(feat)  # <<< KEY CHANGE
                h = backbone(feat); logits = head(h)
...
def train_eval_fold(backbone, head_init, train_grids, train_labels,
    ...
    for epoch in range(S2_EPOCHS):
        ...
        for start in range(0, n_train, S2_BATCH_SIZE):
            ...
            feat = target_ri(x)
            feat = zscore_features(feat)  # <<< KEY CHANGE
            h = backbone_copy(feat); logits = head(h)
            loss = compute_loss(logits, y, mixup_labels=mixup_y, mixup_lam=mixup_lam)
    ...
    train_emb = extract_embeddings(backbone_copy, target_ri, train_grids, apply_zscore=True)
    ...
    if read_ins is not None and all_data is not None and SOURCE_KNN_WEIGHT > 0:
        ...
        for pid in prepare.SOURCE_PATIENTS:
            ...
            emb = extract_embeddings(backbone_copy, ri, all_data[pid]["grids"], apply_zscore=True)
    ...
    if TTA_COPIES > 1:
        val_emb_sum = extract_embeddings(backbone_copy, target_ri, val_grids, apply_zscore=True)
        for _ in range(TTA_COPIES - 1):
            val_emb_sum = val_emb_sum + extract_embeddings(backbone_copy, target_ri, augment(val_grids), apply_zscore=True)
        val_emb = val_emb_sum / TTA_COPIES
    else:
        val_emb = extract_embeddings(backbone_copy, target_ri, val_grids, apply_zscore=True)
...
if __name__ == "__main__":
    ...
    print(f"CHANGE: z-score normalization after read-in (strips patient-specific distributions)")
    print()
