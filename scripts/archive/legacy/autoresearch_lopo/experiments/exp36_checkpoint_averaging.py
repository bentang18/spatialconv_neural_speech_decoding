#!/usr/bin/env python3
"""Exp 36: Backbone Weight Averaging (Stochastic Weight Averaging lite).

Brainstorm Rank 13 — Stage 1 backbone averaging after warmup.

After S1 warmup (epoch 20), save backbone state dict every 10 epochs.
After S1 finishes, average all saved checkpoint weights element-wise.
Use this averaged backbone for S2.

Hypothesis: weight averaging finds a flatter loss-basin center that generalizes
better across patients than the single best-val-loss checkpoint.

Key change: best-checkpoint early stopping kept for head/read_ins, but
backbone gets the average of checkpoints from epoch 20 onwards.

S2 and eval unchanged with prototype/knn/simpleshot eval.
...
S1_EVAL_EVERY = 10
S1_VAL_FRACTION = 0.2
# Checkpoint averaging config
CKPT_START_EPOCH = 20   # start saving checkpoints after warmup
...
# ============================================================
# STAGE 1 — with periodic backbone checkpoint averaging
# ============================================================
...
def train_stage1(all_data):
    ...
    # Backbone checkpoints for averaging
    ...
    for epoch in range(S1_EPOCHS):
        ...
        # Save backbone checkpoint for averaging (after warmup, every CKPT_INTERVAL epochs)
        if epoch >= CKPT_START_EPOCH and (epoch + 1) % CKPT_INTERVAL == 0:
            backbone_checkpoints.append(deepcopy(backbone.state_dict()))
            print(f"  S1 epoch {epoch+1}: saved backbone checkpoint #{len(backbone_checkpoints)}")
        ...
        if (epoch + 1) % S1_EVAL_EVERY == 0:
            ...
            # Save best for head and read_ins (not backbone — will be averaged)
    ...
    if backbone_checkpoints:
        ...
        for key in backbone_checkpoints[0]:
            avg_state[key] = torch.stack(
                [ckpt[key].float() for ckpt in backbone_checkpoints]
            ).mean(dim=0)
        backbone.load_state_dict(avg_state)
        print("  Backbone weight averaging done.")
    else:
        print("  WARNING: No checkpoints collected for averaging (training ended before warmup+interval).")

    # Restore best head and read_ins
    if best_state:
...
if __name__ == "__main__":
    ...
    print(f"CHANGE: Backbone checkpoint averaging every {CKPT_INTERVAL} epochs after epoch {CKPT_START_EPOCH}")
    print()

    print("=== Stage 1: Source patient training (with backbone averaging) ===")
    s1_start = time.time()
