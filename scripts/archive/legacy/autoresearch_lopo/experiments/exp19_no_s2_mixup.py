#!/usr/bin/env python3
"""exp19: No Stage 2 mixup + multi-patient k-NN (clean combination).

Hypothesis: exp11 (no S2 mixup, 0.769) and exp13 (multi-pt kNN, 0.762) were both
improvements. exp16 tried combining them but also changed S2 augmentation, which hurt.
This experiment: ONLY remove S2 mixup from exp13 baseline (keep all other S2 augmentation).

Changes from exp13 baseline:
  - MIXUP_ALPHA_S2 = 0.0 (no mixup in Stage 2 only)
  - Stage 1 still uses mixup (MIXUP_ALPHA = 0.2)
...
LABEL_SMOOTHING = 0.1
FOCAL_GAMMA = 2.0
MIXUP_ALPHA = 0.2      # Stage 1 mixup
MIXUP_ALPHA_S2 = 0.0   # Stage 2 mixup — DISABLED (was 0.2)
TTA_COPIES = 16
KNN_K = 10
...
# ============================================================
# STAGE 1 (identical to baseline — mixup still on)
# ============================================================
...
def train_eval_fold(backbone, head_init, train_grids, train_labels,
    ...
    for epoch in range(S2_EPOCHS):
        ...
        for start in range(0, n_train, S2_BATCH_SIZE):
            ...
            # NO MIXUP in Stage 2 — this is the key change
            optimizer.zero_grad()
            feat = target_ri(x)
            h = backbone_copy(feat)
            logits = head(h)
            loss = compute_loss(logits, y)  # no mixup_labels
...
if __name__ == "__main__":
    ...
    print(f"EXP19: No Stage 2 mixup + multi-patient k-NN")
    print()
    ...
    print("=== Stage 2: Target adaptation (NO MIXUP) + evaluation ===")
