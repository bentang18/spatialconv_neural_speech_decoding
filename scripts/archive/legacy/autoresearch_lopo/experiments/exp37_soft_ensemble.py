#!/usr/bin/env python3
"""Exp 37: Calibrated Soft Ensemble of Eval Methods.

Brainstorm Rank 9 — pure eval change, zero training modification.

Run all eval methods (linear, k-NN, prototype, simpleshot) and create
soft probability ensembles. Instead of hard argmax per method and picking
the best, convert all to probability distributions and average them:

  - Linear logits: softmax → (B, 3, 9) probs
  - k-NN: similarity-weighted class accumulation per position → softmax
  - Prototype: cosine similarity to centroids → softmax
  - SimpleShot: cosine similarity to centered+L2 centroids → softmax

Ensemble options:
  1. ensemble_all: average probs from all 4 methods
  2. ensemble_knn_proto: average k-NN + prototype (two best methods)

Training is IDENTICAL to exp25 baseline. Only eval changes.
...
ENSEMBLE_TEMPERATURE = 1.0  # temperature for softmax over similarities
...
# ============================================================
# EVAL HELPERS — soft probability versions
# ============================================================
...
def knn_soft_probs(train_emb, train_labels, val_emb, k=KNN_K, temperature=ENSEMBLE_TEMPERATURE):
    """Return soft probability distributions (B, 3, 9) from k-NN."""
    ...
    all_pos_probs = []
    for pos in range(prepare.N_POSITIONS):
        # Accumulate similarity-weighted class votes
        raw_scores = torch.zeros(B, prepare.N_CLASSES)
        for i in range(B):
            for j_idx in range(k):
                j = topk_idx[i, j_idx].item()
                cls = train_labels[j][pos] - 1  # 0-indexed
                raw_scores[i, cls] += topk_sim[i, j_idx].item()
        # Softmax over accumulated scores
        probs = F.softmax(raw_scores / max(temperature, 1e-8), dim=1)
        all_pos_probs.append(probs)
    return torch.stack(all_pos_probs, dim=1)  # (B, 3, 9)

def prototype_soft_probs(train_emb, train_labels, val_emb, temperature=ENSEMBLE_TEMPERATURE):
    """Return soft probability distributions (B, 3, 9) from prototype centroids."""
    ...
    all_pos_probs = []
    for pos in range(prepare.N_POSITIONS):
        sims = torch.zeros(B, prepare.N_CLASSES)
        for cls in range(prepare.N_CLASSES):
            mask = [j for j, l in enumerate(train_labels) if l[pos] == cls + 1]
            if not mask:
                continue
            centroid = F.normalize(train_norm[mask].mean(dim=0), dim=0)
            sims[:, cls] = (val_norm * centroid.unsqueeze(0)).sum(dim=1)
        probs = F.softmax(sims / max(temperature, 1e-8), dim=1)
        all_pos_probs.append(probs)
    return torch.stack(all_pos_probs, dim=1)  # (B, 3, 9)

def simpleshot_soft_probs(train_emb, train_labels, val_emb, temperature=ENSEMBLE_TEMPERATURE):
    """Return soft probability distributions (B, 3, 9) from SimpleShot centroids."""
...
def probs_to_preds(probs):
    """Convert (B, 3, 9) probs tensor to list of [p1, p2, p3] 1-indexed predictions."""
    argmax = probs.argmax(dim=-1) + 1  # (B, 3), 1-indexed
    return argmax.tolist()

# Hard predict helpers for individual method reporting
def knn_predict(train_emb, train_labels, val_emb, k=KNN_K):
    probs = knn_soft_probs(train_emb, train_labels, val_emb, k)
    return probs_to_preds(probs)

def simpleshot_predict(train_emb, train_labels, val_emb):
    probs = simpleshot_soft_probs(train_emb, train_labels, val_emb)
    return probs_to_preds(probs)

def prototype_predict(train_emb, train_labels, val_emb):
    probs = prototype_soft_probs(train_emb, train_labels, val_emb)
    return probs_to_preds(probs)
...
# ============================================================
# STAGE 2 + EVAL with soft ensemble
# ============================================================
...
def train_eval_fold(backbone, head_init, train_grids, train_labels,
    ...
    # Linear soft probs: softmax over (B, 3, 9) logits
    linear_probs = F.softmax(logits.cpu(), dim=-1)  # (B, 3, 9)
    linear_preds = probs_to_preds(linear_probs)
    ...
    # Individual method soft probs
    knn_probs = knn_soft_probs(train_emb_all, train_labels_all, val_emb)     # (B, 3, 9)
    proto_probs = prototype_soft_probs(train_emb, list(train_labels), val_emb)  # (B, 3, 9)
    ss_probs = simpleshot_soft_probs(train_emb, list(train_labels), val_emb)    # (B, 3, 9)

    # Hard predictions for individual method reporting
    knn_preds = probs_to_preds(knn_probs)
    knn_per = prepare.compute_per(knn_preds, val_labels)

    proto_preds = probs_to_preds(proto_probs)
    proto_per = prepare.compute_per(proto_preds, val_labels)

    ss_preds = probs_to_preds(ss_probs)
    ss_per = prepare.compute_per(ss_preds, val_labels)

    # Soft ensemble: average all 4 probability distributions
    ensemble_all_probs = (linear_probs + knn_probs + proto_probs + ss_probs) / 4.0
    ensemble_all_preds = probs_to_preds(ensemble_all_probs)
    ensemble_all_per = prepare.compute_per(ensemble_all_preds, val_labels)

    # Soft ensemble: k-NN + prototype only (two strongest methods)
    ensemble_knn_proto_probs = (knn_probs + proto_probs) / 2.0
    ensemble_knn_proto_preds = probs_to_preds(ensemble_knn_proto_probs)
    ensemble_knn_proto_per = prepare.compute_per(ensemble_knn_proto_preds, val_labels)

    # Soft ensemble: k-NN + prototype + simpleshot
    ensemble_3_probs = (knn_probs + proto_probs + ss_probs) / 3.0
    ensemble_3_preds = probs_to_preds(ensemble_3_probs)
    ensemble_3_per = prepare.compute_per(ensemble_3_preds, val_labels)

    methods = {
        "linear": (linear_per, linear_preds),
        "knn": (knn_per, knn_preds),
        "prototype": (proto_per, proto_preds),
        "simpleshot": (ss_per, ss_preds),
        "ensemble_all": (ensemble_all_per, ensemble_all_preds),
        "ensemble_knn_proto": (ensemble_knn_proto_per, ensemble_knn_proto_preds),
        "ensemble_3": (ensemble_3_per, ensemble_3_preds),
    }
...
if __name__ == "__main__":
    ...
    print(f"CHANGE: Soft ensemble of all eval methods (linear+knn+proto+simpleshot)")
    print()
    ...
    method_pers = {m: [] for m in ["linear", "knn", "prototype", "simpleshot",
                                    "ensemble_all", "ensemble_knn_proto", "ensemble_3"]}
    ...
    for fi, (tr_idx, va_idx) in enumerate(splits):
        ...
        print(f"  Fold {fi+1}/{len(splits)}: PER={per:.4f} "
              ...
              f"ens_all={methods['ensemble_all'][0]:.4f} "
              f"ens_kp={methods['ensemble_knn_proto'][0]:.4f} "
              f"ens3={methods['ensemble_3'][0]:.4f} best={best_method})  ({time.time()-ft0:.1f}s)")
