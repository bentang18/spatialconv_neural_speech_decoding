#!/usr/bin/env python3
"""Test: does disabling cuDNN bring CUDA closer to CPU?
If cuDNN is the divergence source, disabling it should reduce the gap."""
import os
import sys
import time
...
# Disable cuDNN — forces PyTorch native GRU (matches CPU)
torch.backends.cudnn.enabled = False
# NOTE: NOT using torch.use_deterministic_algorithms because
# adaptive_avg_pool2d_backward doesn't have a deterministic CUDA implementation
...
# Now run the actual train.py pipeline
...
# ============================================================
# Paste minimal version of train.py here
# ============================================================
...
def train_stage1(all_data):
    ...
    for epoch in range(S1_EPOCHS):
        ...
        for pid in pids:
            ...
            tr_idx = source_train[pid]; perm = np.random.permutation(len(tr_idx))
        ...
        if (epoch + 1) % S1_EVAL_EVERY == 0:
            ...
            print(f"  S1 epoch {epoch+1}: train={avg_loss:.4f}, val={val_loss:.4f}")
...
if __name__ == "__main__":
    ...
    print(f"Device: {DEVICE}  |  cuDNN: DISABLED  |  deterministic: ON")
    print()
    ...
    for fi, (tr_idx, va_idx) in enumerate(splits):
        ...
        print(f"  Fold {fi+1}: PER={per:.4f} (linear={lp:.4f}, kNN={kp:.4f})  ({time.time()-ft0:.1f}s)")
