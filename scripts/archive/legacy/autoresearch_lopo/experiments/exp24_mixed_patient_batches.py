#!/usr/bin/env python3
"""Exp 24: Mixed-patient batches in Stage 1.

Brainstorm Rank 1 — highest expected impact.

KEY INSIGHT: Current S1 iterates patients sequentially — each gradient step
sees only one patient. The backbone can learn patient-specific shortcuts.

FIX: Sample 2-3 patients per gradient step. Process through their respective
read-ins, concatenate the features (both d=256), compute loss jointly.
Single backward pass sees mixed-patient gradients.

CUDA baseline: 0.823 | Target: < 0.80
"""
...
# Stage 1
S1_EPOCHS = 200
S1_BATCH_SIZE = 16  # total batch size across mixed patients
S1_LR = 1e-3
...
S1_EVAL_EVERY = 10
S1_VAL_FRACTION = 0.2
PATIENTS_PER_STEP = 3  # <<< NEW: mix 3 patients per gradient step
...
# ============================================================
# STAGE 1: MIXED-PATIENT BATCHES (KEY CHANGE)
# ============================================================

def train_stage1(all_data):
    """Train with mixed-patient batches — each step sees PATIENTS_PER_STEP patients."""
    ...
    # Pre-build per-patient sample iterators
    patient_iters = {}  # pid -> shuffled index iterator

    def get_patient_batch(pid, batch_size):
        """Get next batch_size samples from this patient (with reshuffling)."""
        ...
        return grids[batch_idx], [labels[i] for i in batch_idx]

    # Estimate steps per epoch: total training samples / batch_size
    ...
    for epoch in range(S1_EPOCHS):
        ...
        for step in range(steps_per_epoch):
            # Sample PATIENTS_PER_STEP random patients for this step
            ...
            # Collect features from each patient through their read-ins
            all_feats = []
            all_labels = []
            for pid in step_pids:
                g, l = get_patient_batch(pid, samples_per_patient)
                g = augment(g).to(DEVICE)
                feat = read_ins[pid](g)  # (B_i, d_flat, T)
                all_feats.append(feat)
                all_labels.extend(l)

            # Concatenate along batch dimension — all now d=256
            feats = torch.cat(all_feats, dim=0)

            # Apply mixup across the mixed batch (cross-patient!)
            ...
            if MIXUP_ALPHA > 0 and feats.shape[0] > 1:
                ...
                perm_mix = torch.randperm(feats.shape[0])
                feats = mixup_lam * feats + (1 - mixup_lam) * feats[perm_mix]
                mixup_labels = [labels_batch[i] for i in perm_mix.tolist()]
...
# ============================================================
# STAGE 2 + EVAL (unchanged from baseline)
# ============================================================
...
if __name__ == "__main__":
    ...
    print(f"CHANGE: Mixed-patient batches, {PATIENTS_PER_STEP} patients per step")
    print()

    print("=== Stage 1: Mixed-patient source training ===")
    s1_start = time.time()
