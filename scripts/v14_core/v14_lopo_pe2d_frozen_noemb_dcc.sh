#!/bin/bash
# Stage-1 close-out: noemb LOPO at 4-core (atlas-mechanism isolation).
#
# Identical to v14_lopo_pe2d_frozen_dcc.sh but with --no-parcel-embedding.
# Tests whether the soft parcel embedding (`support @ P_emb`) actually earns
# its LOPO keep. Must be run at Stage-1 scale — Stage 2 adds SSL + 2× patients
# simultaneously, conflating atlas-mechanism with scale.
#
# Compared against the T3.3_frozen LOPO (job 45769655, LOPO 0.807 ± 0.015).
# If noemb matches within noise → parcel embedding provides no LOPO signal
# at this scale. If noemb drops → embedding is load-bearing.
#
# 60 jobs total (4 held-out × 5 folds × 3 seeds).
#
#SBATCH --job-name=v14pp_lopo_pe2d_frozen_noemb
...
echo "[$(date -Is)] task=${task_id} LOPO held_out=${held_out} pretrain=${PRETRAIN_PATIENTS} fold=${fold} seed=${seed} arm=per_cell+partial_conv+pe2d_frozen+noemb"
nvidia-smi || true
...
echo "[$(date -Is)] === stage 1: pretrain (per_cell + partial_conv + pe2d_frozen + noemb) ==="
...
echo "[$(date -Is)] === stage 2: finetune on ${held_out} (per_cell + partial_conv + pe2d_frozen + noemb) ==="
${PYTHON} scripts/v14_core/train_v14_core.py \
    --mode per-phoneme \
    --patient "${held_out}" \
    --fold "${fold}" \
