#!/bin/bash
# LOPO pretrain → per-patient finetune on the per_cell (canonical) arm.
#
# Mirrors v14_lopo_pretrain_finetune_dcc.sh but drops --temporal-frontend flat,
# so both stages use the canonical per_cell + attention + atlas parcel
# embedding path. This is the informative warm-start test: pooled joint
# training on per_cell does show cross-patient lift per-subject
# (Q1a 4-core: S26 −7.6pp, S33 −10.9pp vs per-patient per_cell),
# so a warm-started backbone has a real handle to transfer.
#
# For each (held-out patient H, fold f, seed s):
#   1. Pretrain pooled per_cell on the 3 non-H core patients using fold f /
#      seed s. Saves best-val state_dict to <PRETRAIN_DIR>/<tag>.ckpt.pt.
#   2. Finetune per-subject on H fold f seed s, loading that checkpoint
#      as initialization.
#
...
#
#SBATCH --job-name=v14pp_lopo_percell
...
ROOT=/work/ht203/results/v14_lopo/per_cell_d32_depth${DEPTH}
...
echo "[$(date -Is)] task=${task_id} LOPO held_out=${held_out} pretrain=${PRETRAIN_PATIENTS} fold=${fold} seed=${seed} arm=per_cell"
nvidia-smi || true
...
echo "[$(date -Is)] === stage 1: pretrain (per_cell) ==="
...
echo "[$(date -Is)] === stage 2: finetune on ${held_out} (per_cell) ==="
${PYTHON} scripts/v14_core/train_v14_core.py \
    --mode per-phoneme \
    --patient "${held_out}" \
    --fold "${fold}" \
