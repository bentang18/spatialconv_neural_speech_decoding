#!/bin/bash
# LOPO pretrain → per-patient finetune.
#
# For each (held-out patient H, fold f, seed s):
#   1. Pretrain pooled flat on the 3 non-H core patients using fold f /
...
#      as initialization. Writes result JSON in <FINETUNE_DIR>.
#
# Tests "does a warm-started backbone beat random init?" — the actual
# pretrain→finetune question that pooled joint-training doesn't answer.
#
# 60 jobs total (4 held-out × 5 folds × 3 seeds).
# Task-ID encoding: task_id = pt_idx * 15 + fold * 3 + seed
#   pt_idx ∈ {0: S14, 1: S26, 2: S33, 3: S62}
#
#SBATCH --job-name=v14pp_lopo_flat
...
ROOT=/work/ht203/results/v14_lopo/flat_d32_depth${DEPTH}
...
echo "[$(date -Is)] task=${task_id} LOPO held_out=${held_out} pretrain=${PRETRAIN_PATIENTS} fold=${fold} seed=${seed}"
nvidia-smi || true

cd "${REPO}"

# Pretrain tag matches the pooled runner's internal tag (sorted patient slug).
