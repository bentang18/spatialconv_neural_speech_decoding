...
* ``--mode per-phoneme`` (default, baseline-aligned): phoneme-level `.fif`,
  flat per-phoneme CE, ``backbone_depth`` ablation knob. Full Phase-1 LH
  cohort supported.
* ``--mode pooled``: multi-patient per-phoneme (Q1a).

The original ``--mode slot`` (trial-level slot-CE over 3 phoneme positions)
was retired in the pre-Stage-0 reorg; its dataset + fold runner now live
under ``src/speech_decoding/archive/exploration/`` and are not importable
from the active CLI.
"""
...
from speech_decoding.studies.cogan_ps.dataset import V14PhonemeDataset  # noqa: E402
from speech_decoding.training.run_fold import (  # noqa: E402
    run_one_fold as run_one_fold_per_phoneme,
)
from speech_decoding.training.run_fold_pooled import (  # noqa: E402
    run_one_fold_pooled,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--mode",
        choices=("per-phoneme", "pooled"),
        default="per-phoneme",
        help="Training mode. 'per-phoneme' = single-patient baseline-aligned; "
        "'pooled' = multi-patient per-phoneme (Q1a).",
    )
    parser.add_argument(
        "--patient",
        default=None,
        help="Required for per-phoneme mode. Ignored for pooled mode.",
    )
    parser.add_argument(
        "--patients",
        default=None,
