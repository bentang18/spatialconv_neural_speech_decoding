#!/bin/bash
# LOPO pretrain → per-patient finetune on per_cell + hierarchical readout.
#
# Same chain as v14_lopo_pretrain_finetune_per_cell_dcc.sh, with
# --readout-mode hierarchical added to both stages. Tests whether the
# factored (cell × time) attention-pool readout — two learned queries,
# temporal pool per cell then cell pool — transfers the backbone to
# held-out patients when composed with the warm-start pattern.
#
# For each (held-out patient H, fold f, seed s):
#   1. Pretrain pooled per_cell + hierarchical on the 3 non-H core
#      patients using fold f / seed s. Saves best-val state_dict.
#   2. Finetune per-subject per_cell + hierarchical on H fold f seed s,
...
#
#SBATCH --job-name=v14pp_lopo_hierarchical
...
ROOT=/work/ht203/results/v14_lopo/per_cell_hierarchical_d32_depth${DEPTH}
...
echo "[$(date -Is)] task=${task_id} LOPO held_out=${held_out} pretrain=${PRETRAIN_PATIENTS} fold=${fold} seed=${seed} arm=per_cell+hierarchical"
nvidia-smi || true
...
echo "[$(date -Is)] === stage 1: pretrain (per_cell + hierarchical) ==="
...
echo "[$(date -Is)] === stage 2: finetune on ${held_out} (per_cell + hierarchical) ==="
${PYTHON} scripts/v14_core/train_v14_core.py \
    --mode per-phoneme \
    --patient "${held_out}" \
    --fold "${fold}" \
