...
# per_cell + partial_conv masking + col_only pe2d (W × d broadcast, no row axis).
#
# Tests whether the grid's col axis alone carries the signal pe2d captures.
#
# 60 jobs total (4 held-out × 5 folds × 3 seeds).
#
#SBATCH --job-name=v14pp_lopo_pe2d_col
...
ROOT=/work/ht203/results/v14_lopo/per_cell_partialconv_pecol_d32_depth${DEPTH}
...
echo "[$(date -Is)] task=${task_id} LOPO held_out=${held_out} pretrain=${PRETRAIN_PATIENTS} fold=${fold} seed=${seed} arm=per_cell+partial_conv+pe_col_only"
nvidia-smi || true
...
echo "[$(date -Is)] === stage 1: pretrain (per_cell + partial_conv + col_only) ==="
...
echo "[$(date -Is)] === stage 2: finetune on ${held_out} (per_cell + partial_conv + col_only) ==="
${PYTHON} scripts/v14_core/train_v14_core.py \
    --mode per-phoneme \
    --patient "${held_out}" \
    --fold "${fold}" \
