#!/bin/bash
# LOPO pretrain → per-patient finetune for T3.3 variant:
# per_cell + partial_conv masking + factorized_2d_frozen pe2d (random, no grad).
#
# Decisive test of whether learned pe2d weights carry structure or act as
# generic bias. If frozen matches learned within seed noise, default can
# drop the learnable layer.
#
# 60 jobs total (4 held-out × 5 folds × 3 seeds).
#
#SBATCH --job-name=v14pp_lopo_pe2d_frozen
...
ROOT=/work/ht203/results/v14_lopo/per_cell_partialconv_pe2dfrozen_d32_depth${DEPTH}
...
echo "[$(date -Is)] task=${task_id} LOPO held_out=${held_out} pretrain=${PRETRAIN_PATIENTS} fold=${fold} seed=${seed} arm=per_cell+partial_conv+pe2d_frozen"
nvidia-smi || true
...
echo "[$(date -Is)] === stage 1: pretrain (per_cell + partial_conv + pe2d_frozen) ==="
...
echo "[$(date -Is)] === stage 2: finetune on ${held_out} (per_cell + partial_conv + pe2d_frozen) ==="
${PYTHON} scripts/v14_core/train_v14_core.py \
    --mode per-phoneme \
    --patient "${held_out}" \
    --fold "${fold}" \
