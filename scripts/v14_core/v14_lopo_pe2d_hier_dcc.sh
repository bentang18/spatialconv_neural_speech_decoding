#!/bin/bash
# LOPO pretrain → per-patient finetune for T3.4 composition:
# per_cell + partial_conv masking + factorized_2d pe2d + hierarchical readout.
#
# Tests whether the hierarchical readout compounds with pe2d on the LOPO
# protocol or just duplicates it. If pooled-joint shows a positive delta
# vs pe2d-alone (0.823) and LOPO either matches or improves that delta,
# hierarchical stays as the default readout.
#
# 60 jobs total (4 held-out × 5 folds × 3 seeds).
#
#SBATCH --job-name=v14pp_lopo_pe2d_hier
...
ROOT=/work/ht203/results/v14_lopo/per_cell_partialconv_pe2d_hier_d32_depth${DEPTH}
...
echo "[$(date -Is)] task=${task_id} LOPO held_out=${held_out} pretrain=${PRETRAIN_PATIENTS} fold=${fold} seed=${seed} arm=per_cell+partial_conv+pe2d+hierarchical"
nvidia-smi || true
...
echo "[$(date -Is)] === stage 1: pretrain (per_cell + partial_conv + pe2d + hierarchical) ==="
...
echo "[$(date -Is)] === stage 2: finetune on ${held_out} (per_cell + partial_conv + pe2d + hierarchical) ==="
${PYTHON} scripts/v14_core/train_v14_core.py \
    --mode per-phoneme \
    --patient "${held_out}" \
    --fold "${fold}" \
